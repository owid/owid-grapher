...
export type SidecarExampleFlavour = "archie" | "archie-document"

/** The prose field of a sidecar an example sits in (see SidecarProse) */
export type SidecarSectionKey = "intro" | "whenToUse" | "whenNotToUse" | "notes"

/**
 * One fenced example of a sidecar, identified by where it sits rather than
 * by its text: the prose section and its position among that section's
 * fences. The client matches the n-th fence it renders in a section to the
 * example with that position, so two fences with identical source stay
 * distinct.
 */
export interface SidecarExample {
    archie: string
    flavour: SidecarExampleFlavour
    section: SidecarSectionKey
    /** Zero-based position among the fences of `section` */
    position: number
}

export type ComponentExample = SidecarExample

/** What a `{.id}` / `{guide:id}` / `{template:id}` mention points at */
export type RelatedRefKind = "component" | "guide" | "template"

/**
 * A cross-reference harvested from a sidecar's prose by the generator, which
 * fails the build when the id resolves to nothing — so the client can link
...
export interface ComponentReference {
    ...
    system?: boolean
    /**
     * Produced by the parser from Google Docs formatting rather than
     * written as an ArchieML tag — the value names what formatting produces
     * it (e.g. "a plain paragraph", "a line with a Heading style"). Authors
     * meet these blocks without choosing them, so they sort after the
     * explicit blocks of their category, and the detail page shows a
     * warning instead of authoring guidance.
     */
    autoGenerated?: string
    /** Cross-references harvested from the decision prose ("choose this vs …") */
    related?: RelatedRef[]
