...
- `featuredWork`: The four featured tiles, authored as a
  `[.featured-work]` section with exactly four entries (any other count
  is a parse error). Each entry needs a `url:` — a gdoc, grapher, or
  explorer link resolves title, description, authors, and thumbnail
  automatically, while a plain external URL must supply `title:` and
  `authors:` explicitly (and, for the 1st and 3rd tiles, a `filename:`
  thumbnail); entries breaking these rules are dropped from the grid
  with a parse error. `kicker:` is free text shown above the tile's
  title (e.g. "Article - 10 Min Read", "Announcement") — no strong
  conventions yet. `isNew: true` shows a red "NEW" pill before that
  tile's kicker.
