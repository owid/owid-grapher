...
- `left`: The narrower (5 of 12) scrolling text column, authored as a
  `[.+left]` freeform section. If it starts with a heading, that heading
  is lifted out and rendered full-width above both columns (unless the
  right column starts with a chart story). When the layout collapses on
  tablets and phones this column comes first. Omitted, it is left empty.
- `right`: The wider sticky column, authored as `[.+right]` — usually the
  chart or visual. It pins near the top of the viewport while the reader
  scrolls the left column; on tablets and phones it stops sticking and
  moves below the left column. The order of the two sections in the doc
  doesn't matter — the section names decide the columns.
