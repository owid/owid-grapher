...
- `content`: Everything the group shows and hides together, authored as a
  `[.+content]` … `[]` freeform section that can hold any blocks —
  typically a heading followed by one or more `{.data-callout}` blocks.
  Required, and it must contain at least one `{.data-callout}`; without
  that the block is dropped.
