...
- `url`: The chart to show — a Grapher, explorer or MDIM URL. Query params
  ...
  dimension params shows its default view; add the view's dimension query
  params (e.g. `?metric=unvaccinated&antigen=comparison`) to select one
  specific view instead.
...
For an MDIM, prefer creating a narrative chart over overriding its title:
select the view you want on its datapage, open the share menu, and choose
