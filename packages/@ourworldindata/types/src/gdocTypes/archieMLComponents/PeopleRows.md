...
- `columns`: How many columns the grid has on desktop, `2` or `4`.
  Four columns suit compact cards; two suit cards with longer bios.
  Fewer columns render on smaller screens either way. Required — any
  other value falls back to a single column.
- `people`: The person cards filling the grid, authored as an inner
  `[.+people]` … `[]` list of `{.person}` blocks (see `{.person}` for
  the fields on each). Required.
