...
- `title`: The small-caps heading shown next to the pills. Required;
  without it the block is dropped and reported as a parse error.
- `pills`: The pills themselves, authored as a `[.pills]` section with
  a `url:` line per pill, each optionally preceded by a `text:` line
  for the pill's label. For gdoc URLs `text` can be omitted — the
  gdoc's title is used; for any other URL it is required, and the whole
  block is dropped without it.

## Notes

On author pages, keep the pills to one desktop line (about 5–7 topics).
