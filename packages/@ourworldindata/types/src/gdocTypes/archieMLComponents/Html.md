...
- `value`: The raw HTML, written after the `html:` key (end a multi-line
  snippet with `:end`). It is inserted into the page exactly as written,
  with no validation — broken markup can break the article layout, so
  double-check it in the preview.
