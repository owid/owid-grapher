...
- `datapageUrl`: The grapher URL of the datapage to feature; the
  embedded chart, the indicator name and the "Explore the data" button
  all come from it. It must link to a grapher that is a datapage, and
  can include query parameters like `?time=earliest..latest` to control
  the default view. Required; without it the block is dropped and
  reported as a parse error.
- `title`: The card's narrative headline, phrased as the question the
  indicator answers. Required; without it the block is dropped and
  reported as a parse error.
- `text`: The narrative next to the chart, authored as a `[.+text]`
  section of rich-text paragraphs (links and formatting survive).
  Required; missing, or written without the `[.+text]` tag, the block
  is dropped and reported as a parse error.
- `source`: A short source attribution shown after the indicator name.
  Omitted, the indicator's own short attribution from the datapage is
  shown instead.
