...
- `title`: The heading shown next to the chart icon at the top of the
  section. Omitted, "Explore the data" is shown.
- `align`: `left` (what you get when it is omitted) keeps the header
  left-aligned; `center` centers the header and also centers headings
  and text inside the section. Any other value keeps `left` and reports
  a parse error.
- `content`: The wrapped blocks, authored as a `[.+content]` freeform
  section — typically guided charts, sticky containers, or explorers.
  Required.
