...
- `items`: The full content. There is no named key — the block is itself
  a freeform section, so everything between `[.+expandable-paragraph]`
  and the closing `[]` is the content, and any blocks are allowed. Only
  the first few lines are visible until the reader clicks "Show more".
  An empty section is dropped and reported as a parse error.
