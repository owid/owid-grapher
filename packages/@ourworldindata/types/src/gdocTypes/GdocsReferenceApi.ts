...
// resolved exemplar documents of a template.
//
 ...
 * The UI leads with these words, backed by the percentage; raw counts are
 * tooltip material.
 */
...
/**
 * The single place the thresholds above are applied. It lives beside the
 * label list so the server (usage.json, instances.json) and the admin client
 * (form frequency, per-prop adoption) cannot word the same fraction
 * differently on the same page.
 */
export function componentUsageLabel(fraction: number): ComponentUsageLabel {
...
export interface ComponentInstancesResponse {
    ...
    /**
     * Per-property default, as the parser fills it: the value a scanned
     * instance carries for a prop its minimal source omits — the most common
     * one when instances disagree (configs extracted under older parsers).
     * Only scalar defaults; a prop that stays undefined when omitted is absent.
     */
    propDefaults: Record<string, string>
}
/**
 * One exemplar of a template — an editorially chosen published document —
 * resolved live so the page can render it and link to it.
 */
export interface ExemplarOutline {
    /** Id of the published gdoc — addresses the rendered document preview */
    gdocId: string
    slug: string
    title: string
    docType: OwidGdocType
    publishedAt: string | null
    /**
     * For a profile template, the entity slug of the first entity the
     * profile's scope resolves to — profiles bake per-entity at
     * `/profile/<slug>/<entitySlug>`, so the live link needs it to avoid a
     * 404 on the bare template slug. Absent when no entity resolves.
     */
    entitySlug?: string
}
