    ...
    implements ChartInterface, HorizontalColorLegendManager
{
    ...
    @computed private get legendHeightWithPadding(): number {
        if (!this.showLegend || this.legend.height === 0) return 0
        return this.legend.height + this.legendPadding
    }

    @computed private get facetsContainerBounds(): Bounds {
        // TODO: padTop is for the labels
        return this.bounds.padTop(24).padBottom(this.legendHeightWithPadding)
    }
    ...
    @computed get series(): FacetSeries[] {
        return [this.manager.startTime, this.manager.endTime].map((time) => {
            ...
            return {
                seriesName: time?.toString() ?? "Missing time",
