...
export class TimelineController {
    ...
    @computed get shouldUseEqualSpacing(): boolean {
        ...
        const gaps = R.zipWith(
            this.timesAsc.slice(1),
            this.timesAsc,
            (curr, prev) => curr - prev
        )

        const sortedGaps = R.sortBy(gaps, (gap) => -gap)

        // Use equal spacing if gaps are highly concentrated (top decile > 50%)
        const topDecileCount = Math.max(1, Math.floor(gaps.length * 0.1))
        const topDecileSum = R.sumBy(
            R.take(sortedGaps, topDecileCount),
            R.identity()
        )
    ...
    private timeToIndex(time: Time): number {
        return this.timesAsc.indexOf(time)
    }
