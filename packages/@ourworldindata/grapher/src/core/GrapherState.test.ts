...
describe("prepareTableForDownload", () => {
    ...
    it("attempts to fill in missing entity codes from inputTable", () => {
        ...
        const table = new OwidTable([
            ...
            ["United Kingdom", "", 2, 2021, 3600], // Missing code in one row
        ])
        ...
        // At least one UK row should have the code
        expect(ukRows.length).toBeGreaterThan(0)
        expect(ukRows.some((r) => r.entityCode === "GBR")).toBe(true)
    })
    ...
    it("replaces duplicate times with missing value placeholders in original time columns", () => {
        // Create a table with tolerance already applied
        let table = new OwidTable(
            [
                ["entityName", "entityCode", "year", "gdp"],
                ["France", "FRA", 2019, 2500],
        ...
        // Apply tolerance to create the originalTime column
        table = table.interpolateColumnWithTolerance("gdp")
        ...
        // When the original time equals the main time, it should be replaced
        // with a missing value placeholder
        const originalTimes =
            downloadTable.get("gdp-originalTime").valuesIncludingErrorValues
        const times = downloadTable.get("year").values

        // Find an index where originalTime === time
        const matchingIndex = times.findIndex(
            (time, i) => time === originalTimes[i]
        )

        if (matchingIndex >= 0) {
            // That matching original time should be replaced with a placeholder
            expect(originalTimes[matchingIndex]).toBe(
