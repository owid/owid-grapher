...
- `yarn startDemoServer` serves this directory on http://localhost:8433 (a plain static server, so the demo pages load `dist/` exactly like a CDN consumer would). Open `/demo.html` for the three `GrapherLoader` variants or `/core-econ-demo.html` for a styled embedding example.
