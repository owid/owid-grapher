...
describe("the 'short' abbreviation bands", () => {
    ...
    const cases: [string, number, string, TickFormattingOptions][] = [
        ...
        ["sig-fig column below its derived threshold", 25_240, "25,200", { roundingMode: OwidVariableRoundingMode.significantFigures }],
        ["sig-fig column keeps round values below the threshold written out", 26_000, "26,000", { roundingMode: OwidVariableRoundingMode.significantFigures }],
        ["sig-fig column abbreviates from 10^(sf+2)", 123_456, "123k", { roundingMode: OwidVariableRoundingMode.significantFigures }],
        ...
        ["2 sig figs write out below 10k", 3_000, "3,000", { roundingMode: OwidVariableRoundingMode.significantFigures, numSignificantFigures: 2 }],
        ["2 sig figs abbreviate from 10k", 26_000, "26k", { roundingMode: OwidVariableRoundingMode.significantFigures, numSignificantFigures: 2 }],
        ["4 sig figs abbreviate only from 1M", 123_456, "123,500", { roundingMode: OwidVariableRoundingMode.significantFigures, numSignificantFigures: 4 }],
    ]
    cases.forEach(([description, input, output, options]) => {
        it(description, () => {
            expect(
