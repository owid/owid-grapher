...
import { removeTrailingParenthetical, traverseEnrichedBlock } from "./Util.js"

export const PROFILE_ENTITY_CODE = "$entityCode"
export const PROFILE_ENTITY_NAME = "$entityName"
export const PROFILE_NO_ARTICLE_ENTITY_NAME = "$noArticleEntityName"
...
export const instantiateProfile = (
    ...
    const replacements: ReplacementPair[] = [
        [PROFILE_ENTITY_NAME, entityNameWithArticle],
        [PROFILE_ENTITY_CODE, entity.code],
        [PROFILE_NO_ARTICLE_ENTITY_NAME, entity.name],
    ]
    ...
    clonedContent.body.forEach((node) => {
        traverseEnrichedBlock(node, (block) => {
            if (block.type === "conditional-section") {
                parseInstantiatedConditionalSection(block)
            }
        })
    })
...
function parseInstantiatedConditionalSection(
    ...
    const result = checkShouldConditionalSectionRender({
        entity: block.entity,
        include: block.include,
        exclude: block.exclude,
    })
    block.parseErrors.push(...result.errors)
...
export function checkShouldConditionalSectionRender({
    ...
    include: string[]
    exclude: string[]
}): { shouldRender: boolean; errors: ParseError[] } {
    const entityRegion = getRegionByName(entity)
    if (!entityRegion) {
        return {
            shouldRender: false,
            errors: [{ message: `Unknown region: ${entity}` }],
        }
    }
    ...
    const shouldRender = calculateRenderLogic(
        entityRegion,
        includeRegions,
        excludeRegions
    )

    return { shouldRender, errors: [] }
}
