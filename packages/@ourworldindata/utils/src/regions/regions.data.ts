// This file is auto-generated by devTools/regionsUpdater/update.ts
// Do not edit manually. Run `yarn runRegionsUpdater` to regenerate.

export const regionsData = [
  { code: "ABW", shortCode: "AW", name: "Aruba", slug: "aruba", regionType: "country" },
  {
    code: "AFG",
    shortCode: "AF",
    name: "Afghanistan",
    slug: "afghanistan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "AGO",
    shortCode: "AO",
    name: "Angola",
    slug: "angola",
    regionType: "country",
    isMappable: true,
  },
  { code: "AIA", shortCode: "AI", name: "Anguilla", slug: "anguilla", regionType: "country" },
  {
    code: "ALA",
    shortCode: "AX",
    name: "Aland Islands",
    slug: "aland-islands",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "ALB",
    shortCode: "AL",
    name: "Albania",
    slug: "albania",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "AND",
    shortCode: "AD",
    name: "Andorra",
    slug: "andorra",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ANT",
    shortCode: "AN",
    name: "Netherlands Antilles",
    slug: "netherlands-antilles",
    regionType: "country",
    isHistorical: true,
    isUnlisted: true,
  },
  {
    code: "ARE",
    shortCode: "AE",
    name: "United Arab Emirates",
    slug: "united-arab-emirates",
    regionType: "country",
    isMappable: true,
    variantNames: ["UAE"],
  },
  {
    code: "ARG",
    shortCode: "AR",
    name: "Argentina",
    slug: "argentina",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ARM",
    shortCode: "AM",
    name: "Armenia",
    slug: "armenia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ASM",
    shortCode: "AS",
    name: "American Samoa",
    slug: "american-samoa",
    regionType: "country",
  },
  {
    code: "ATA",
    shortCode: "AQ",
    name: "Antarctica",
    slug: "antarctica",
    regionType: "other",
    isMappable: true,
    isUnlisted: true,
  },
  {
    code: "ATF",
    shortCode: "TF",
    name: "French Southern Territories",
    slug: "french-southern-territories",
    regionType: "country",
    isMappable: true,
    isUnlisted: true,
  },
  {
    code: "ATG",
    shortCode: "AG",
    name: "Antigua and Barbuda",
    slug: "antigua-and-barbuda",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "AUS",
    shortCode: "AU",
    name: "Australia",
    slug: "australia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "AUT",
    shortCode: "AT",
    name: "Austria",
    slug: "austria",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "AZE",
    shortCode: "AZ",
    name: "Azerbaijan",
    slug: "azerbaijan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BDI",
    shortCode: "BI",
    name: "Burundi",
    slug: "burundi",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BEL",
    shortCode: "BE",
    name: "Belgium",
    slug: "belgium",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BEN",
    shortCode: "BJ",
    name: "Benin",
    slug: "benin",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BES",
    shortCode: "BQ",
    name: "Bonaire Sint Eustatius and Saba",
    slug: "bonaire-sint-eustatius-and-saba",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "BFA",
    shortCode: "BF",
    name: "Burkina Faso",
    slug: "burkina-faso",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BGD",
    shortCode: "BD",
    name: "Bangladesh",
    slug: "bangladesh",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BGR",
    shortCode: "BG",
    name: "Bulgaria",
    slug: "bulgaria",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BHR",
    shortCode: "BH",
    name: "Bahrain",
    slug: "bahrain",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BHS",
    shortCode: "BS",
    name: "Bahamas",
    slug: "bahamas",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BIH",
    shortCode: "BA",
    name: "Bosnia and Herzegovina",
    shortName: "Bosnia and Herz.",
    slug: "bosnia-and-herzegovina",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BLM",
    shortCode: "BL",
    name: "Saint Barthelemy",
    slug: "saint-barthelemy",
    regionType: "country",
  },
  {
    code: "BLR",
    shortCode: "BY",
    name: "Belarus",
    slug: "belarus",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BLZ",
    shortCode: "BZ",
    name: "Belize",
    slug: "belize",
    regionType: "country",
    isMappable: true,
  },
  { code: "BMU", shortCode: "BM", name: "Bermuda", slug: "bermuda", regionType: "country" },
  {
    code: "BOL",
    shortCode: "BO",
    name: "Bolivia",
    slug: "bolivia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BRA",
    shortCode: "BR",
    name: "Brazil",
    slug: "brazil",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BRB",
    shortCode: "BB",
    name: "Barbados",
    slug: "barbados",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BRN",
    shortCode: "BN",
    name: "Brunei",
    slug: "brunei",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BTN",
    shortCode: "BT",
    name: "Bhutan",
    slug: "bhutan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "BVT",
    shortCode: "BV",
    name: "Bouvet Island",
    slug: "bouvet-island",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "BWA",
    shortCode: "BW",
    name: "Botswana",
    slug: "botswana",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CAF",
    shortCode: "CF",
    name: "Central African Republic",
    slug: "central-african-republic",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CAN",
    shortCode: "CA",
    name: "Canada",
    slug: "canada",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CCK",
    shortCode: "CC",
    name: "Cocos Islands",
    slug: "cocos-islands",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "CHE",
    shortCode: "CH",
    name: "Switzerland",
    slug: "switzerland",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CHL",
    shortCode: "CL",
    name: "Chile",
    slug: "chile",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CHN",
    shortCode: "CN",
    name: "China",
    slug: "china",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CIV",
    shortCode: "CI",
    name: "Cote d'Ivoire",
    slug: "cote-divoire",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CMR",
    shortCode: "CM",
    name: "Cameroon",
    slug: "cameroon",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "COD",
    shortCode: "CD",
    name: "Democratic Republic of Congo",
    shortName: "DR Congo",
    slug: "democratic-republic-of-congo",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "COG",
    shortCode: "CG",
    name: "Congo",
    slug: "congo",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "COK",
    shortCode: "CK",
    name: "Cook Islands",
    slug: "cook-islands",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "COL",
    shortCode: "CO",
    name: "Colombia",
    slug: "colombia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "COM",
    shortCode: "KM",
    name: "Comoros",
    slug: "comoros",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CPV",
    shortCode: "CV",
    name: "Cape Verde",
    slug: "cape-verde",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CRI",
    shortCode: "CR",
    name: "Costa Rica",
    slug: "costa-rica",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CUB",
    shortCode: "CU",
    name: "Cuba",
    slug: "cuba",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CUW",
    shortCode: "CW",
    name: "Curacao",
    slug: "curacao",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "CXR",
    shortCode: "CX",
    name: "Christmas Island",
    slug: "christmas-island",
    regionType: "country",
  },
  {
    code: "CYM",
    shortCode: "KY",
    name: "Cayman Islands",
    slug: "cayman-islands",
    regionType: "country",
  },
  {
    code: "CYP",
    shortCode: "CY",
    name: "Cyprus",
    slug: "cyprus",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "CZE",
    shortCode: "CZ",
    name: "Czechia",
    slug: "czechia",
    regionType: "country",
    isMappable: true,
    variantNames: ["Czech Republic"],
  },
  {
    code: "DEU",
    shortCode: "DE",
    name: "Germany",
    slug: "germany",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "DJI",
    shortCode: "DJ",
    name: "Djibouti",
    slug: "djibouti",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "DMA",
    shortCode: "DM",
    name: "Dominica",
    slug: "dominica",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "DNK",
    shortCode: "DK",
    name: "Denmark",
    slug: "denmark",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "DOM",
    shortCode: "DO",
    name: "Dominican Republic",
    slug: "dominican-republic",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "DZA",
    shortCode: "DZ",
    name: "Algeria",
    slug: "algeria",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ECU",
    shortCode: "EC",
    name: "Ecuador",
    slug: "ecuador",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "EGY",
    shortCode: "EG",
    name: "Egypt",
    slug: "egypt",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ERI",
    shortCode: "ER",
    name: "Eritrea",
    slug: "eritrea",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ESH",
    shortCode: "EH",
    name: "Western Sahara",
    slug: "western-sahara",
    regionType: "country",
    isMappable: true,
    isUnlisted: true,
  },
  {
    code: "ESP",
    shortCode: "ES",
    name: "Spain",
    slug: "spain",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "EST",
    shortCode: "EE",
    name: "Estonia",
    slug: "estonia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ETH",
    shortCode: "ET",
    name: "Ethiopia",
    slug: "ethiopia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "FIN",
    shortCode: "FI",
    name: "Finland",
    slug: "finland",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "FJI",
    shortCode: "FJ",
    name: "Fiji",
    slug: "fiji",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "FLK",
    shortCode: "FK",
    name: "Falkland Islands",
    slug: "falkland-islands",
    regionType: "country",
  },
  {
    code: "FRA",
    shortCode: "FR",
    name: "France",
    slug: "france",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "FRO",
    shortCode: "FO",
    name: "Faroe Islands",
    slug: "faroe-islands",
    regionType: "country",
  },
  {
    code: "FSM",
    shortCode: "FM",
    name: "Micronesia (country)",
    slug: "micronesia-country",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GAB",
    shortCode: "GA",
    name: "Gabon",
    slug: "gabon",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GBR",
    shortCode: "GB",
    name: "United Kingdom",
    slug: "united-kingdom",
    regionType: "country",
    isMappable: true,
    variantNames: ["UK"],
  },
  {
    code: "GEO",
    shortCode: "GE",
    name: "Georgia",
    slug: "georgia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GGY",
    shortCode: "GG",
    name: "Guernsey",
    slug: "guernsey",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "GHA",
    shortCode: "GH",
    name: "Ghana",
    slug: "ghana",
    regionType: "country",
    isMappable: true,
  },
  { code: "GIB", shortCode: "GI", name: "Gibraltar", slug: "gibraltar", regionType: "country" },
  {
    code: "GIN",
    shortCode: "GN",
    name: "Guinea",
    slug: "guinea",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GLP",
    shortCode: "GP",
    name: "Guadeloupe",
    slug: "guadeloupe",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "GMB",
    shortCode: "GM",
    name: "Gambia",
    slug: "gambia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GNB",
    shortCode: "GW",
    name: "Guinea-Bissau",
    slug: "guinea-bissau",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GNQ",
    shortCode: "GQ",
    name: "Equatorial Guinea",
    slug: "equatorial-guinea",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GRC",
    shortCode: "GR",
    name: "Greece",
    slug: "greece",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GRD",
    shortCode: "GD",
    name: "Grenada",
    slug: "grenada",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GRL",
    shortCode: "GL",
    name: "Greenland",
    slug: "greenland",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GTM",
    shortCode: "GT",
    name: "Guatemala",
    slug: "guatemala",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "GUF",
    shortCode: "GF",
    name: "French Guiana",
    slug: "french-guiana",
    regionType: "country",
    isMappable: true,
  },
  { code: "GUM", shortCode: "GU", name: "Guam", slug: "guam", regionType: "country" },
  {
    code: "GUY",
    shortCode: "GY",
    name: "Guyana",
    slug: "guyana",
    regionType: "country",
    isMappable: true,
  },
  { code: "HKG", shortCode: "HK", name: "Hong Kong", slug: "hong-kong", regionType: "country" },
  {
    code: "HMD",
    shortCode: "HM",
    name: "Heard Island and McDonald Islands",
    slug: "heard-island-and-mcdonald-islands",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "HND",
    shortCode: "HN",
    name: "Honduras",
    slug: "honduras",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "HRV",
    shortCode: "HR",
    name: "Croatia",
    slug: "croatia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "HTI",
    shortCode: "HT",
    name: "Haiti",
    slug: "haiti",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "HUN",
    shortCode: "HU",
    name: "Hungary",
    slug: "hungary",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "IDN",
    shortCode: "ID",
    name: "Indonesia",
    slug: "indonesia",
    regionType: "country",
    isMappable: true,
  },
  { code: "IMN", shortCode: "IM", name: "Isle of Man", slug: "isle-of-man", regionType: "country" },
  {
    code: "IND",
    shortCode: "IN",
    name: "India",
    slug: "india",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "IOT",
    shortCode: "IO",
    name: "British Indian Ocean Territory",
    slug: "british-indian-ocean-territory",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "IRL",
    shortCode: "IE",
    name: "Ireland",
    slug: "ireland",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "IRN",
    shortCode: "IR",
    name: "Iran",
    slug: "iran",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "IRQ",
    shortCode: "IQ",
    name: "Iraq",
    slug: "iraq",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ISL",
    shortCode: "IS",
    name: "Iceland",
    slug: "iceland",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ISR",
    shortCode: "IL",
    name: "Israel",
    slug: "israel",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ITA",
    shortCode: "IT",
    name: "Italy",
    slug: "italy",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "JAM",
    shortCode: "JM",
    name: "Jamaica",
    slug: "jamaica",
    regionType: "country",
    isMappable: true,
  },
  { code: "JEY", shortCode: "JE", name: "Jersey", slug: "jersey", regionType: "country" },
  {
    code: "JOR",
    shortCode: "JO",
    name: "Jordan",
    slug: "jordan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "JPN",
    shortCode: "JP",
    name: "Japan",
    slug: "japan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KAZ",
    shortCode: "KZ",
    name: "Kazakhstan",
    slug: "kazakhstan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KEN",
    shortCode: "KE",
    name: "Kenya",
    slug: "kenya",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KGZ",
    shortCode: "KG",
    name: "Kyrgyzstan",
    slug: "kyrgyzstan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KHM",
    shortCode: "KH",
    name: "Cambodia",
    slug: "cambodia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KIR",
    shortCode: "KI",
    name: "Kiribati",
    slug: "kiribati",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KNA",
    shortCode: "KN",
    name: "Saint Kitts and Nevis",
    slug: "saint-kitts-and-nevis",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KOR",
    shortCode: "KR",
    name: "South Korea",
    slug: "south-korea",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "KWT",
    shortCode: "KW",
    name: "Kuwait",
    slug: "kuwait",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LAO",
    shortCode: "LA",
    name: "Laos",
    slug: "laos",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LBN",
    shortCode: "LB",
    name: "Lebanon",
    slug: "lebanon",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LBR",
    shortCode: "LR",
    name: "Liberia",
    slug: "liberia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LBY",
    shortCode: "LY",
    name: "Libya",
    slug: "libya",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LCA",
    shortCode: "LC",
    name: "Saint Lucia",
    slug: "saint-lucia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LIE",
    shortCode: "LI",
    name: "Liechtenstein",
    slug: "liechtenstein",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LKA",
    shortCode: "LK",
    name: "Sri Lanka",
    slug: "sri-lanka",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LSO",
    shortCode: "LS",
    name: "Lesotho",
    slug: "lesotho",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LTU",
    shortCode: "LT",
    name: "Lithuania",
    slug: "lithuania",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LUX",
    shortCode: "LU",
    name: "Luxembourg",
    slug: "luxembourg",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "LVA",
    shortCode: "LV",
    name: "Latvia",
    slug: "latvia",
    regionType: "country",
    isMappable: true,
  },
  { code: "MAC", shortCode: "MO", name: "Macao", slug: "macao", regionType: "country" },
  {
    code: "MAF",
    shortCode: "MF",
    name: "Saint Martin (French part)",
    slug: "saint-martin-french-part",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "MAR",
    shortCode: "MA",
    name: "Morocco",
    slug: "morocco",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MCO",
    shortCode: "MC",
    name: "Monaco",
    slug: "monaco",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MDA",
    shortCode: "MD",
    name: "Moldova",
    slug: "moldova",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MDG",
    shortCode: "MG",
    name: "Madagascar",
    slug: "madagascar",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MDV",
    shortCode: "MV",
    name: "Maldives",
    slug: "maldives",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MEX",
    shortCode: "MX",
    name: "Mexico",
    slug: "mexico",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MHL",
    shortCode: "MH",
    name: "Marshall Islands",
    slug: "marshall-islands",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MKD",
    shortCode: "MK",
    name: "North Macedonia",
    slug: "north-macedonia",
    regionType: "country",
    isMappable: true,
    variantNames: ["Macedonia"],
  },
  {
    code: "MLI",
    shortCode: "ML",
    name: "Mali",
    slug: "mali",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MLT",
    shortCode: "MT",
    name: "Malta",
    slug: "malta",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MMR",
    shortCode: "MM",
    name: "Myanmar",
    slug: "myanmar",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MNE",
    shortCode: "ME",
    name: "Montenegro",
    slug: "montenegro",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MNG",
    shortCode: "MN",
    name: "Mongolia",
    slug: "mongolia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MNP",
    shortCode: "MP",
    name: "Northern Mariana Islands",
    slug: "northern-mariana-islands",
    regionType: "country",
  },
  {
    code: "MOZ",
    shortCode: "MZ",
    name: "Mozambique",
    slug: "mozambique",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MRT",
    shortCode: "MR",
    name: "Mauritania",
    slug: "mauritania",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MSR",
    shortCode: "MS",
    name: "Montserrat",
    slug: "montserrat",
    regionType: "country",
    isUnlisted: true,
  },
  { code: "MTQ", shortCode: "MQ", name: "Martinique", slug: "martinique", regionType: "country" },
  {
    code: "MUS",
    shortCode: "MU",
    name: "Mauritius",
    slug: "mauritius",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MWI",
    shortCode: "MW",
    name: "Malawi",
    slug: "malawi",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "MYS",
    shortCode: "MY",
    name: "Malaysia",
    slug: "malaysia",
    regionType: "country",
    isMappable: true,
  },
  { code: "MYT", shortCode: "YT", name: "Mayotte", slug: "mayotte", regionType: "country" },
  {
    code: "NAM",
    shortCode: "NA",
    name: "Namibia",
    slug: "namibia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NCL",
    shortCode: "NC",
    name: "New Caledonia",
    slug: "new-caledonia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NER",
    shortCode: "NE",
    name: "Niger",
    slug: "niger",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NFK",
    shortCode: "NF",
    name: "Norfolk Island",
    slug: "norfolk-island",
    regionType: "country",
  },
  {
    code: "NGA",
    shortCode: "NG",
    name: "Nigeria",
    slug: "nigeria",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NIC",
    shortCode: "NI",
    name: "Nicaragua",
    slug: "nicaragua",
    regionType: "country",
    isMappable: true,
  },
  { code: "NIU", shortCode: "NU", name: "Niue", slug: "niue", regionType: "country" },
  {
    code: "NLD",
    shortCode: "NL",
    name: "Netherlands",
    slug: "netherlands",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NOR",
    shortCode: "NO",
    name: "Norway",
    slug: "norway",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NPL",
    shortCode: "NP",
    name: "Nepal",
    slug: "nepal",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NRU",
    shortCode: "NR",
    name: "Nauru",
    slug: "nauru",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "NZL",
    shortCode: "NZ",
    name: "New Zealand",
    slug: "new-zealand",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "OMN",
    shortCode: "OM",
    name: "Oman",
    slug: "oman",
    regionType: "country",
    isMappable: true,
  },
  { code: "OWID_ABK", name: "Abkhazia", slug: "abkhazia", regionType: "other" },
  {
    code: "OWID_AFR",
    name: "Africa",
    slug: "africa",
    regionType: "continent",
    translationCodes: ["002"],
    members: [
      "AGO",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "DJI",
      "DZA",
      "EGY",
      "ERI",
      "ESH",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "KEN",
      "LBR",
      "LBY",
      "LSO",
      "MAR",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "MYT",
      "NAM",
      "NER",
      "NGA",
      "OWID_ERE",
      "OWID_OFS",
      "OWID_SDN",
      "OWID_SML",
      "OWID_ZAN",
      "REU",
      "RWA",
      "SDN",
      "SEN",
      "SHN",
      "SLE",
      "SOM",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TUN",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "OWID_AKD",
    name: "Akrotiri and Dhekelia",
    slug: "akrotiri-and-dhekelia",
    regionType: "other",
  },
  {
    code: "OWID_ASI",
    name: "Asia",
    slug: "asia",
    regionType: "continent",
    translationCodes: ["142"],
    members: [
      "AFG",
      "ARE",
      "ARM",
      "AZE",
      "BGD",
      "BHR",
      "BRN",
      "BTN",
      "CCK",
      "CHN",
      "CXR",
      "GEO",
      "HKG",
      "IDN",
      "IND",
      "IOT",
      "IRN",
      "IRQ",
      "ISR",
      "JOR",
      "JPN",
      "KAZ",
      "KGZ",
      "KHM",
      "KOR",
      "KWT",
      "LAO",
      "LBN",
      "LKA",
      "MAC",
      "MDV",
      "MMR",
      "MNG",
      "MYS",
      "NPL",
      "OMN",
      "OWID_ABK",
      "OWID_CYN",
      "OWID_DRV",
      "OWID_FPK",
      "OWID_KRU",
      "OWID_NAG",
      "OWID_RVN",
      "OWID_SOS",
      "OWID_YAR",
      "OWID_YPR",
      "PAK",
      "PHL",
      "PRK",
      "PSE",
      "QAT",
      "SAU",
      "SGP",
      "SYR",
      "THA",
      "TJK",
      "TKM",
      "TLS",
      "TUR",
      "TWN",
      "UZB",
      "VNM",
      "YEM",
    ],
  },
  {
    code: "OWID_AUH",
    name: "Austria-Hungary",
    slug: "austria-hungary",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_BAD",
    name: "Grand Duchy of Baden",
    slug: "grand-duchy-of-baden",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_BAV",
    name: "Kingdom of Bavaria",
    slug: "kingdom-of-bavaria",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_CIS",
    name: "Channel Islands",
    slug: "channel-islands",
    regionType: "aggregate",
    members: ["GGY", "JEY"],
  },
  { code: "OWID_CYN", name: "Northern Cyprus", slug: "northern-cyprus", regionType: "other" },
  {
    code: "OWID_CZS",
    name: "Czechoslovakia",
    slug: "czechoslovakia",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_DRV",
    name: "Democratic Republic of Vietnam",
    slug: "democratic-republic-of-vietnam",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_DSI",
    name: "Kingdom of the Two Sicilies",
    slug: "kingdom-of-the-two-sicilies",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_ERE",
    name: "Ethiopia (former)",
    slug: "ethiopia-former",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_EU27",
    name: "European Union (27)",
    slug: "european-union-27",
    regionType: "aggregate",
    members: [
      "AUT",
      "BEL",
      "BGR",
      "CYP",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "GRC",
      "HRV",
      "HUN",
      "IRL",
      "ITA",
      "LTU",
      "LUX",
      "LVA",
      "MLT",
      "NLD",
      "POL",
      "PRT",
      "ROU",
      "SVK",
      "SVN",
      "SWE",
    ],
  },
  {
    code: "OWID_EUR",
    name: "Europe",
    slug: "europe",
    regionType: "continent",
    translationCodes: ["150"],
    members: [
      "ALA",
      "ALB",
      "AND",
      "AUT",
      "BEL",
      "BGR",
      "BIH",
      "BLR",
      "CHE",
      "CYP",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "FRO",
      "GBR",
      "GGY",
      "GIB",
      "GRC",
      "HRV",
      "HUN",
      "IMN",
      "IRL",
      "ISL",
      "ITA",
      "JEY",
      "LIE",
      "LTU",
      "LUX",
      "LVA",
      "MCO",
      "MDA",
      "MKD",
      "MLT",
      "MNE",
      "NLD",
      "NOR",
      "OWID_AKD",
      "OWID_AUH",
      "OWID_BAD",
      "OWID_BAV",
      "OWID_CZS",
      "OWID_DSI",
      "OWID_GDR",
      "OWID_GFR",
      "OWID_KOS",
      "OWID_MOD",
      "OWID_PAR",
      "OWID_SAR",
      "OWID_SAX",
      "OWID_SEK",
      "OWID_SRM",
      "OWID_TRS",
      "OWID_TSC",
      "OWID_USS",
      "OWID_WTB",
      "OWID_YGS",
      "POL",
      "PRT",
      "ROU",
      "RUS",
      "SJM",
      "SMR",
      "SRB",
      "SVK",
      "SVN",
      "SWE",
      "UKR",
      "VAT",
    ],
  },
  {
    code: "OWID_FPK",
    name: "Pakistan (former)",
    slug: "pakistan-former",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_GCL",
    name: "Great Colombia",
    slug: "great-colombia",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_GDR",
    name: "East Germany",
    slug: "east-germany",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_GFR",
    name: "West Germany",
    slug: "west-germany",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_HIC",
    name: "High-income countries",
    slug: "high-income-countries",
    regionType: "income_group",
    members: [
      "ABW",
      "AND",
      "ARE",
      "ASM",
      "ATF",
      "ATG",
      "AUS",
      "AUT",
      "BEL",
      "BGR",
      "BHR",
      "BHS",
      "BMU",
      "BRB",
      "BRN",
      "CAN",
      "CHE",
      "CHL",
      "CRI",
      "CUW",
      "CYM",
      "CYP",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "FRO",
      "GBR",
      "GIB",
      "GRC",
      "GRL",
      "GUF",
      "GUM",
      "GUY",
      "HKG",
      "HRV",
      "HUN",
      "IMN",
      "IRL",
      "ISL",
      "ISR",
      "ITA",
      "JPN",
      "KNA",
      "KOR",
      "KWT",
      "LIE",
      "LTU",
      "LUX",
      "LVA",
      "MAC",
      "MAF",
      "MCO",
      "MLT",
      "MNP",
      "NCL",
      "NLD",
      "NOR",
      "NRU",
      "NZL",
      "OMN",
      "OWID_CIS",
      "PAN",
      "PLW",
      "POL",
      "PRI",
      "PRT",
      "PYF",
      "QAT",
      "ROU",
      "RUS",
      "SAU",
      "SGP",
      "SMR",
      "SVK",
      "SVN",
      "SWE",
      "SXM",
      "SYC",
      "TCA",
      "TTO",
      "TWN",
      "URY",
      "USA",
      "VGB",
      "VIR",
    ],
  },
  { code: "OWID_KOS", name: "Kosovo", slug: "kosovo", regionType: "country", isMappable: true },
  {
    code: "OWID_KRU",
    name: "Korea (former)",
    slug: "korea-former",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_LIC",
    name: "Low-income countries",
    slug: "low-income-countries",
    regionType: "income_group",
    members: [
      "AFG",
      "BDI",
      "BFA",
      "CAF",
      "COD",
      "ERI",
      "GMB",
      "GNB",
      "LBR",
      "MDG",
      "MLI",
      "MOZ",
      "MWI",
      "NER",
      "PRK",
      "RWA",
      "SDN",
      "SLE",
      "SOM",
      "SSD",
      "SYR",
      "TCD",
      "TGO",
      "UGA",
      "YEM",
    ],
  },
  {
    code: "OWID_LMC",
    name: "Lower-middle-income countries",
    slug: "lower-middle-income-countries",
    regionType: "income_group",
    members: [
      "AGO",
      "BEN",
      "BGD",
      "BOL",
      "BTN",
      "CIV",
      "CMR",
      "COG",
      "COM",
      "DJI",
      "EGY",
      "FSM",
      "GHA",
      "GIN",
      "HND",
      "HTI",
      "IND",
      "JOR",
      "KEN",
      "KGZ",
      "KHM",
      "KIR",
      "LAO",
      "LBN",
      "LKA",
      "LSO",
      "MAR",
      "MMR",
      "MRT",
      "NAM",
      "NGA",
      "NIC",
      "NPL",
      "PAK",
      "PHL",
      "PNG",
      "PSE",
      "SEN",
      "SLB",
      "STP",
      "SWZ",
      "TJK",
      "TLS",
      "TUN",
      "TZA",
      "UZB",
      "VNM",
      "VUT",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "OWID_MNS",
    name: "Melanesia",
    slug: "melanesia",
    regionType: "aggregate",
    translationCodes: ["054"],
    members: ["FJI", "PNG", "SLB", "VUT"],
  },
  {
    code: "OWID_MOD",
    name: "Duchy of Modena and Reggio",
    slug: "duchy-of-modena-and-reggio",
    regionType: "country",
    isHistorical: true,
  },
  { code: "OWID_NAG", name: "Nagorno-Karabakh", slug: "nagorno-karabakh", regionType: "other" },
  {
    code: "OWID_NAM",
    name: "North America",
    slug: "north-america",
    regionType: "continent",
    translationCodes: ["003"],
    members: [
      "ABW",
      "AIA",
      "ANT",
      "ATG",
      "BES",
      "BHS",
      "BLM",
      "BLZ",
      "BMU",
      "BRB",
      "CAN",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "GLP",
      "GRD",
      "GRL",
      "GTM",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MAF",
      "MEX",
      "MSR",
      "MTQ",
      "NIC",
      "OWID_RCA",
      "PAN",
      "PRI",
      "SLV",
      "SPM",
      "SXM",
      "TCA",
      "TTO",
      "USA",
      "VCT",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "OWID_NH",
    name: "Northern Hemisphere",
    slug: "northern-hemisphere",
    regionType: "aggregate",
    members: [
      "ABW",
      "AFG",
      "AIA",
      "ALA",
      "ALB",
      "AND",
      "ANT",
      "ARE",
      "ARM",
      "ATG",
      "AUT",
      "AZE",
      "BEL",
      "BEN",
      "BES",
      "BFA",
      "BGD",
      "BGR",
      "BHR",
      "BHS",
      "BIH",
      "BLM",
      "BLR",
      "BLZ",
      "BMU",
      "BRB",
      "BRN",
      "BTN",
      "CAF",
      "CAN",
      "CHE",
      "CHN",
      "CIV",
      "CMR",
      "COL",
      "CPV",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "CYP",
      "CZE",
      "DEU",
      "DJI",
      "DMA",
      "DNK",
      "DOM",
      "DZA",
      "EGY",
      "ERI",
      "ESH",
      "ESP",
      "EST",
      "ETH",
      "FIN",
      "FRA",
      "FRO",
      "FSM",
      "GBR",
      "GEO",
      "GGY",
      "GHA",
      "GIB",
      "GIN",
      "GLP",
      "GMB",
      "GNB",
      "GNQ",
      "GRC",
      "GRD",
      "GRL",
      "GTM",
      "GUF",
      "GUM",
      "GUY",
      "HKG",
      "HND",
      "HRV",
      "HTI",
      "HUN",
      "IMN",
      "IND",
      "IRL",
      "IRN",
      "IRQ",
      "ISL",
      "ISR",
      "ITA",
      "JAM",
      "JEY",
      "JOR",
      "JPN",
      "KAZ",
      "KEN",
      "KGZ",
      "KHM",
      "KIR",
      "KNA",
      "KOR",
      "KWT",
      "LAO",
      "LBN",
      "LBR",
      "LBY",
      "LCA",
      "LIE",
      "LKA",
      "LTU",
      "LUX",
      "LVA",
      "MAC",
      "MAF",
      "MAR",
      "MCO",
      "MDA",
      "MDV",
      "MEX",
      "MHL",
      "MKD",
      "MLI",
      "MLT",
      "MMR",
      "MNE",
      "MNG",
      "MNP",
      "MRT",
      "MSR",
      "MTQ",
      "MYS",
      "NER",
      "NGA",
      "NIC",
      "NLD",
      "NOR",
      "NPL",
      "OMN",
      "OWID_AUH",
      "OWID_BAD",
      "OWID_BAV",
      "OWID_CZS",
      "OWID_DRV",
      "OWID_DSI",
      "OWID_ERE",
      "OWID_FPK",
      "OWID_GCL",
      "OWID_GDR",
      "OWID_GFR",
      "OWID_KOS",
      "OWID_KRU",
      "OWID_MOD",
      "OWID_PAR",
      "OWID_RCA",
      "OWID_RVN",
      "OWID_SAR",
      "OWID_SAX",
      "OWID_SDN",
      "OWID_SRM",
      "OWID_TSC",
      "OWID_USS",
      "OWID_WTB",
      "OWID_YAR",
      "OWID_YGS",
      "OWID_YPR",
      "PAK",
      "PAN",
      "PHL",
      "PLW",
      "POL",
      "PRI",
      "PRK",
      "PRT",
      "PSE",
      "PS_GZA",
      "QAT",
      "ROU",
      "RUS",
      "SAU",
      "SDN",
      "SEN",
      "SGP",
      "SLE",
      "SLV",
      "SMR",
      "SOM",
      "SPM",
      "SRB",
      "SSD",
      "STP",
      "SUR",
      "SVK",
      "SVN",
      "SWE",
      "SXM",
      "SYR",
      "TCA",
      "TCD",
      "TGO",
      "THA",
      "TJK",
      "TKM",
      "TTO",
      "TUN",
      "TUR",
      "TWN",
      "UGA",
      "UKR",
      "USA",
      "UZB",
      "VAT",
      "VCT",
      "VEN",
      "VGB",
      "VIR",
      "VNM",
      "YEM",
    ],
  },
  {
    code: "OWID_OCE",
    name: "Oceania",
    slug: "oceania",
    regionType: "continent",
    translationCodes: ["009"],
    members: [
      "ASM",
      "AUS",
      "COK",
      "FJI",
      "FSM",
      "GUM",
      "KIR",
      "MHL",
      "MNP",
      "NCL",
      "NFK",
      "NIU",
      "NRU",
      "NZL",
      "PCN",
      "PLW",
      "PNG",
      "PYF",
      "SLB",
      "TKL",
      "TON",
      "TUV",
      "UMI",
      "VUT",
      "WLF",
      "WSM",
    ],
  },
  {
    code: "OWID_OFS",
    name: "Orange Free State",
    slug: "orange-free-state",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_PAR",
    name: "Duchy of Parma and Piacenza",
    slug: "duchy-of-parma-and-piacenza",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_PYA",
    name: "Polynesia",
    slug: "polynesia",
    regionType: "aggregate",
    translationCodes: ["061"],
    members: ["ASM", "COK", "NFK", "NIU", "NZL", "PCN", "PYF", "TKL", "TON", "TUV", "WLF", "WSM"],
  },
  {
    code: "OWID_RCA",
    name: "Federal Republic of Central America",
    slug: "federal-republic-of-central-america",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_RVN",
    name: "Republic of Vietnam",
    slug: "republic-of-vietnam",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_SAM",
    name: "South America",
    slug: "south-america",
    regionType: "continent",
    translationCodes: ["005"],
    members: [
      "ARG",
      "BOL",
      "BRA",
      "CHL",
      "COL",
      "ECU",
      "FLK",
      "GUF",
      "GUY",
      "OWID_GCL",
      "PER",
      "PRY",
      "SGS",
      "SUR",
      "URY",
      "VEN",
    ],
  },
  {
    code: "OWID_SAR",
    name: "Kingdom of Sardinia",
    slug: "kingdom-of-sardinia",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_SAX",
    name: "Kingdom of Saxony",
    slug: "kingdom-of-saxony",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_SDN",
    name: "Sudan (former)",
    slug: "sudan-former",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_SEK",
    name: "Serbia excluding Kosovo",
    slug: "serbia-excluding-kosovo",
    regionType: "other",
  },
  {
    code: "OWID_SH",
    name: "Southern Hemisphere",
    slug: "southern-hemisphere",
    regionType: "aggregate",
    members: [
      "AGO",
      "ARG",
      "ASM",
      "ATF",
      "AUS",
      "BDI",
      "BOL",
      "BRA",
      "BVT",
      "BWA",
      "CCK",
      "CHL",
      "COD",
      "COG",
      "COK",
      "COM",
      "CXR",
      "ECU",
      "FJI",
      "FLK",
      "GAB",
      "HMD",
      "IDN",
      "IOT",
      "LSO",
      "MDG",
      "MOZ",
      "MUS",
      "MWI",
      "MYT",
      "NAM",
      "NCL",
      "NFK",
      "NIU",
      "NRU",
      "NZL",
      "OWID_OFS",
      "PCN",
      "PER",
      "PNG",
      "PRY",
      "PYF",
      "REU",
      "RWA",
      "SGS",
      "SHN",
      "SLB",
      "SWZ",
      "SYC",
      "TKL",
      "TLS",
      "TON",
      "TUV",
      "TZA",
      "URY",
      "VUT",
      "WLF",
      "WSM",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  { code: "OWID_SML", name: "Somaliland", slug: "somaliland", regionType: "other" },
  { code: "OWID_SOS", name: "South Ossetia", slug: "south-ossetia", regionType: "other" },
  {
    code: "OWID_SRM",
    name: "Serbia and Montenegro",
    slug: "serbia-and-montenegro",
    regionType: "country",
    isHistorical: true,
  },
  { code: "OWID_TRS", name: "Transnistria", slug: "transnistria", regionType: "other" },
  {
    code: "OWID_TSC",
    name: "Grand Duchy of Tuscany",
    slug: "grand-duchy-of-tuscany",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_UMC",
    name: "Upper-middle-income countries",
    slug: "upper-middle-income-countries",
    regionType: "income_group",
    members: [
      "ALB",
      "ARG",
      "ARM",
      "AZE",
      "BIH",
      "BLR",
      "BLZ",
      "BRA",
      "BWA",
      "CHN",
      "COL",
      "CPV",
      "CUB",
      "DMA",
      "DOM",
      "DZA",
      "ECU",
      "FJI",
      "GAB",
      "GEO",
      "GNQ",
      "GRD",
      "GTM",
      "IDN",
      "IRN",
      "IRQ",
      "JAM",
      "KAZ",
      "LBY",
      "LCA",
      "MDA",
      "MDV",
      "MEX",
      "MHL",
      "MKD",
      "MNE",
      "MNG",
      "MUS",
      "MYS",
      "OWID_KOS",
      "PER",
      "PRY",
      "SLV",
      "SRB",
      "SUR",
      "THA",
      "TKM",
      "TON",
      "TUR",
      "TUV",
      "UKR",
      "VCT",
      "WSM",
      "ZAF",
    ],
  },
  { code: "OWID_USS", name: "USSR", slug: "ussr", regionType: "country", isHistorical: true },
  {
    code: "OWID_WRL",
    name: "World",
    slug: "world",
    regionType: "aggregate",
    translationCodes: ["001"],
    members: ["OWID_AFR", "OWID_ASI", "OWID_EUR", "OWID_NAM", "OWID_OCE", "OWID_SAM"],
  },
  {
    code: "OWID_WTB",
    name: "Kingdom of Wurttemberg",
    slug: "kingdom-of-wurttemberg",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_YAR",
    name: "Yemen Arab Republic",
    slug: "yemen-arab-republic",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_YGS",
    name: "Yugoslavia",
    slug: "yugoslavia",
    regionType: "country",
    isHistorical: true,
  },
  {
    code: "OWID_YPR",
    name: "Yemen People's Republic",
    slug: "yemen-peoples-republic",
    regionType: "country",
    isHistorical: true,
  },
  { code: "OWID_ZAN", name: "Zanzibar", slug: "zanzibar", regionType: "other" },
  {
    code: "PAK",
    shortCode: "PK",
    name: "Pakistan",
    slug: "pakistan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PAN",
    shortCode: "PA",
    name: "Panama",
    slug: "panama",
    regionType: "country",
    isMappable: true,
  },
  { code: "PCN", shortCode: "PN", name: "Pitcairn", slug: "pitcairn", regionType: "country" },
  {
    code: "PER",
    shortCode: "PE",
    name: "Peru",
    slug: "peru",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PEW_APA",
    name: "Asia-Pacific (Pew)",
    slug: "asia-pacific-pew",
    regionType: "aggregate",
    definedBy: "pew",
    members: [
      "AFG",
      "ARM",
      "ASM",
      "AUS",
      "AZE",
      "BGD",
      "BRN",
      "BTN",
      "CHN",
      "COK",
      "CYP",
      "FJI",
      "FSM",
      "GUM",
      "HKG",
      "IDN",
      "IND",
      "IRN",
      "JPN",
      "KAZ",
      "KGZ",
      "KHM",
      "KIR",
      "KOR",
      "LAO",
      "LKA",
      "MAC",
      "MDV",
      "MHL",
      "MMR",
      "MNG",
      "MNP",
      "MYS",
      "NCL",
      "NIU",
      "NPL",
      "NRU",
      "NZL",
      "PAK",
      "PHL",
      "PLW",
      "PNG",
      "PRK",
      "PYF",
      "SGP",
      "SLB",
      "THA",
      "TJK",
      "TKL",
      "TKM",
      "TLS",
      "TON",
      "TUR",
      "TUV",
      "UZB",
      "VNM",
      "VUT",
      "WLF",
      "WSM",
    ],
  },
  {
    code: "PEW_EUR",
    name: "Europe (Pew)",
    slug: "europe-pew",
    regionType: "aggregate",
    definedBy: "pew",
    members: [
      "ALB",
      "AND",
      "AUT",
      "BEL",
      "BGR",
      "BIH",
      "BLR",
      "CHE",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "FRO",
      "GBR",
      "GEO",
      "GGY",
      "GIB",
      "GRC",
      "HRV",
      "HUN",
      "IMN",
      "IRL",
      "ISL",
      "ITA",
      "JEY",
      "LIE",
      "LTU",
      "LUX",
      "LVA",
      "MCO",
      "MDA",
      "MKD",
      "MLT",
      "MNE",
      "NLD",
      "NOR",
      "POL",
      "PRT",
      "ROU",
      "RUS",
      "SMR",
      "SRB",
      "SVK",
      "SVN",
      "SWE",
      "UKR",
      "VAT",
    ],
  },
  {
    code: "PEW_LAC",
    name: "Latin America-Caribbean (Pew)",
    slug: "latin-america-caribbean-pew",
    regionType: "aggregate",
    definedBy: "pew",
    members: [
      "ABW",
      "AIA",
      "ARG",
      "ATG",
      "BES",
      "BHS",
      "BLZ",
      "BOL",
      "BRA",
      "BRB",
      "CHL",
      "COL",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "ECU",
      "FLK",
      "GLP",
      "GRD",
      "GTM",
      "GUF",
      "GUY",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MEX",
      "MSR",
      "MTQ",
      "NIC",
      "PAN",
      "PER",
      "PRI",
      "PRY",
      "SLV",
      "SUR",
      "SXM",
      "TCA",
      "TTO",
      "URY",
      "VCT",
      "VEN",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "PEW_MENA",
    name: "Middle East-North Africa (Pew)",
    slug: "middle-east-north-africa-pew",
    regionType: "aggregate",
    definedBy: "pew",
    members: [
      "ARE",
      "BHR",
      "DZA",
      "EGY",
      "ESH",
      "IRQ",
      "ISR",
      "JOR",
      "KWT",
      "LBN",
      "LBY",
      "MAR",
      "OMN",
      "PSE",
      "QAT",
      "SAU",
      "SDN",
      "SYR",
      "TUN",
      "YEM",
    ],
  },
  {
    code: "PEW_NAM",
    name: "North America (Pew)",
    slug: "north-america-pew",
    regionType: "aggregate",
    definedBy: "pew",
    members: ["BMU", "CAN", "GRL", "SPM", "USA"],
  },
  {
    code: "PEW_SSA",
    name: "Sub-Saharan Africa (Pew)",
    slug: "sub-saharan-africa-pew",
    regionType: "aggregate",
    definedBy: "pew",
    members: [
      "AGO",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "DJI",
      "ERI",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "KEN",
      "LBR",
      "LSO",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "MYT",
      "NAM",
      "NER",
      "NGA",
      "REU",
      "RWA",
      "SEN",
      "SHN",
      "SLE",
      "SOM",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "PHL",
    shortCode: "PH",
    name: "Philippines",
    slug: "philippines",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PLW",
    shortCode: "PW",
    name: "Palau",
    slug: "palau",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PNG",
    shortCode: "PG",
    name: "Papua New Guinea",
    slug: "papua-new-guinea",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "POL",
    shortCode: "PL",
    name: "Poland",
    slug: "poland",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PRI",
    shortCode: "PR",
    name: "Puerto Rico",
    slug: "puerto-rico",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PRK",
    shortCode: "KP",
    name: "North Korea",
    slug: "north-korea",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PRT",
    shortCode: "PT",
    name: "Portugal",
    slug: "portugal",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PRY",
    shortCode: "PY",
    name: "Paraguay",
    slug: "paraguay",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PSE",
    shortCode: "PS",
    name: "Palestine",
    slug: "palestine",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "PS_GZA",
    shortCode: "PS-GZA",
    name: "Gaza Strip",
    slug: "gaza-strip",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "PYF",
    shortCode: "PF",
    name: "French Polynesia",
    slug: "french-polynesia",
    regionType: "country",
  },
  {
    code: "QAT",
    shortCode: "QA",
    name: "Qatar",
    slug: "qatar",
    regionType: "country",
    isMappable: true,
  },
  { code: "REU", shortCode: "RE", name: "Reunion", slug: "reunion", regionType: "country" },
  {
    code: "ROU",
    shortCode: "RO",
    name: "Romania",
    slug: "romania",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "RUS",
    shortCode: "RU",
    name: "Russia",
    slug: "russia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "RWA",
    shortCode: "RW",
    name: "Rwanda",
    slug: "rwanda",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SAU",
    shortCode: "SA",
    name: "Saudi Arabia",
    slug: "saudi-arabia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SDN",
    shortCode: "SD",
    name: "Sudan",
    slug: "sudan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SEN",
    shortCode: "SN",
    name: "Senegal",
    slug: "senegal",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SGP",
    shortCode: "SG",
    name: "Singapore",
    slug: "singapore",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SGS",
    shortCode: "GS",
    name: "South Georgia and the South Sandwich Islands",
    slug: "south-georgia-and-the-south-sandwich-islands",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "SHN",
    shortCode: "SH",
    name: "Saint Helena",
    slug: "saint-helena",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "SJM",
    shortCode: "SJ",
    name: "Svalbard and Jan Mayen",
    slug: "svalbard-and-jan-mayen",
    regionType: "other",
    isUnlisted: true,
  },
  {
    code: "SLB",
    shortCode: "SB",
    name: "Solomon Islands",
    slug: "solomon-islands",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SLE",
    shortCode: "SL",
    name: "Sierra Leone",
    slug: "sierra-leone",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SLV",
    shortCode: "SV",
    name: "El Salvador",
    slug: "el-salvador",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SMR",
    shortCode: "SM",
    name: "San Marino",
    slug: "san-marino",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SOM",
    shortCode: "SO",
    name: "Somalia",
    slug: "somalia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SPM",
    shortCode: "PM",
    name: "Saint Pierre and Miquelon",
    slug: "saint-pierre-and-miquelon",
    regionType: "country",
  },
  {
    code: "SRB",
    shortCode: "RS",
    name: "Serbia",
    slug: "serbia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SSD",
    shortCode: "SS",
    name: "South Sudan",
    slug: "south-sudan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "STP",
    shortCode: "ST",
    name: "Sao Tome and Principe",
    slug: "sao-tome-and-principe",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SUR",
    shortCode: "SR",
    name: "Suriname",
    slug: "suriname",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SVK",
    shortCode: "SK",
    name: "Slovakia",
    slug: "slovakia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SVN",
    shortCode: "SI",
    name: "Slovenia",
    slug: "slovenia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SWE",
    shortCode: "SE",
    name: "Sweden",
    slug: "sweden",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SWZ",
    shortCode: "SZ",
    name: "Eswatini",
    slug: "eswatini",
    regionType: "country",
    isMappable: true,
    variantNames: ["Swaziland"],
  },
  {
    code: "SXM",
    shortCode: "SX",
    name: "Sint Maarten (Dutch part)",
    slug: "sint-maarten-dutch-part",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "SYC",
    shortCode: "SC",
    name: "Seychelles",
    slug: "seychelles",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "SYR",
    shortCode: "SY",
    name: "Syria",
    slug: "syria",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TCA",
    shortCode: "TC",
    name: "Turks and Caicos Islands",
    shortName: "Turks and Caicos",
    slug: "turks-and-caicos-islands",
    regionType: "country",
  },
  {
    code: "TCD",
    shortCode: "TD",
    name: "Chad",
    slug: "chad",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TGO",
    shortCode: "TG",
    name: "Togo",
    slug: "togo",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "THA",
    shortCode: "TH",
    name: "Thailand",
    slug: "thailand",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TJK",
    shortCode: "TJ",
    name: "Tajikistan",
    slug: "tajikistan",
    regionType: "country",
    isMappable: true,
  },
  { code: "TKL", shortCode: "TK", name: "Tokelau", slug: "tokelau", regionType: "country" },
  {
    code: "TKM",
    shortCode: "TM",
    name: "Turkmenistan",
    slug: "turkmenistan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TLS",
    shortCode: "TL",
    name: "East Timor",
    slug: "east-timor",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TON",
    shortCode: "TO",
    name: "Tonga",
    slug: "tonga",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TTO",
    shortCode: "TT",
    name: "Trinidad and Tobago",
    slug: "trinidad-and-tobago",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TUN",
    shortCode: "TN",
    name: "Tunisia",
    slug: "tunisia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TUR",
    shortCode: "TR",
    name: "Turkey",
    slug: "turkey",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TUV",
    shortCode: "TV",
    name: "Tuvalu",
    slug: "tuvalu",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TWN",
    shortCode: "TW",
    name: "Taiwan",
    slug: "taiwan",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "TZA",
    shortCode: "TZ",
    name: "Tanzania",
    slug: "tanzania",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "UGA",
    shortCode: "UG",
    name: "Uganda",
    slug: "uganda",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "UKR",
    shortCode: "UA",
    name: "Ukraine",
    slug: "ukraine",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "UMI",
    shortCode: "UM",
    name: "United States Minor Outlying Islands",
    slug: "united-states-minor-outlying-islands",
    regionType: "other",
    isUnlisted: true,
  },
  {
    code: "UNM49_AFR",
    name: "Africa (UN M49)",
    slug: "africa-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_1",
    translationCodes: ["002"],
    members: [
      "AGO",
      "ATF",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "DJI",
      "DZA",
      "EGY",
      "ERI",
      "ESH",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "IOT",
      "KEN",
      "LBR",
      "LBY",
      "LSO",
      "MAR",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "MYT",
      "NAM",
      "NER",
      "NGA",
      "REU",
      "RWA",
      "SDN",
      "SEN",
      "SHN",
      "SLE",
      "SOM",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TUN",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "UNM49_AMR",
    name: "Americas (UN M49)",
    slug: "americas-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_1",
    translationCodes: ["019"],
    members: [
      "ABW",
      "AIA",
      "ARG",
      "ATG",
      "BES",
      "BHS",
      "BLM",
      "BLZ",
      "BMU",
      "BOL",
      "BRA",
      "BRB",
      "BVT",
      "CAN",
      "CHL",
      "COL",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "ECU",
      "FLK",
      "GLP",
      "GRD",
      "GRL",
      "GTM",
      "GUF",
      "GUY",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MAF",
      "MEX",
      "MSR",
      "MTQ",
      "NIC",
      "PAN",
      "PER",
      "PRI",
      "PRY",
      "SGS",
      "SLV",
      "SPM",
      "SUR",
      "SXM",
      "TCA",
      "TTO",
      "URY",
      "USA",
      "VCT",
      "VEN",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "UNM49_ANZ",
    name: "Australia and New Zealand (UN M49)",
    slug: "australia-and-new-zealand-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["053"],
    members: ["AUS", "CCK", "CXR", "HMD", "NFK", "NZL"],
  },
  {
    code: "UNM49_ASI",
    name: "Asia (UN M49)",
    slug: "asia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_1",
    translationCodes: ["142"],
    members: [
      "AFG",
      "ARE",
      "ARM",
      "AZE",
      "BGD",
      "BHR",
      "BRN",
      "BTN",
      "CHN",
      "CYP",
      "GEO",
      "HKG",
      "IDN",
      "IND",
      "IRN",
      "IRQ",
      "ISR",
      "JOR",
      "JPN",
      "KAZ",
      "KGZ",
      "KHM",
      "KOR",
      "KWT",
      "LAO",
      "LBN",
      "LKA",
      "MAC",
      "MDV",
      "MMR",
      "MNG",
      "MYS",
      "NPL",
      "OMN",
      "PAK",
      "PHL",
      "PRK",
      "QAT",
      "SAU",
      "SGP",
      "SYR",
      "THA",
      "TJK",
      "TKM",
      "TLS",
      "TUR",
      "UZB",
      "VNM",
      "YEM",
    ],
  },
  {
    code: "UNM49_CAM",
    name: "Central America (UN M49)",
    slug: "central-america-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_3",
    translationCodes: ["013"],
    members: ["BLZ", "CRI", "GTM", "HND", "MEX", "NIC", "PAN", "SLV"],
  },
  {
    code: "UNM49_CAR",
    name: "Caribbean (UN M49)",
    slug: "caribbean-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_3",
    translationCodes: ["029"],
    members: [
      "ABW",
      "AIA",
      "ATG",
      "BES",
      "BHS",
      "BLM",
      "BRB",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "GLP",
      "GRD",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MAF",
      "MSR",
      "MTQ",
      "PRI",
      "SXM",
      "TCA",
      "TTO",
      "VCT",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "UNM49_CAS",
    name: "Central Asia (UN M49)",
    slug: "central-asia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["143"],
    members: ["KAZ", "KGZ", "TJK", "TKM", "UZB"],
  },
  {
    code: "UNM49_EAF",
    name: "Eastern Africa (UN M49)",
    slug: "eastern-africa-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_3",
    translationCodes: ["014"],
    members: [
      "ATF",
      "BDI",
      "COM",
      "DJI",
      "ERI",
      "ETH",
      "IOT",
      "KEN",
      "MDG",
      "MOZ",
      "MUS",
      "MWI",
      "MYT",
      "REU",
      "RWA",
      "SOM",
      "SSD",
      "SYC",
      "TZA",
      "UGA",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "UNM49_EAS",
    name: "Eastern Asia (UN M49)",
    slug: "eastern-asia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["030"],
    members: ["CHN", "HKG", "JPN", "KOR", "MAC", "MNG", "PRK"],
  },
  {
    code: "UNM49_EEU",
    name: "Eastern Europe (UN M49)",
    slug: "eastern-europe-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["151"],
    members: ["BGR", "BLR", "CZE", "HUN", "MDA", "POL", "ROU", "RUS", "SVK", "UKR"],
  },
  {
    code: "UNM49_EUR",
    name: "Europe (UN M49)",
    slug: "europe-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_1",
    translationCodes: ["150"],
    members: [
      "ALA",
      "ALB",
      "AND",
      "AUT",
      "BEL",
      "BGR",
      "BIH",
      "BLR",
      "CHE",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "FRO",
      "GBR",
      "GGY",
      "GIB",
      "GRC",
      "HRV",
      "HUN",
      "IMN",
      "IRL",
      "ISL",
      "ITA",
      "JEY",
      "LIE",
      "LTU",
      "LUX",
      "LVA",
      "MCO",
      "MDA",
      "MKD",
      "MLT",
      "MNE",
      "NLD",
      "NOR",
      "POL",
      "PRT",
      "ROU",
      "RUS",
      "SJM",
      "SMR",
      "SRB",
      "SVK",
      "SVN",
      "SWE",
      "UKR",
      "VAT",
    ],
  },
  {
    code: "UNM49_LAC",
    name: "Latin America and the Caribbean (UN M49)",
    slug: "latin-america-and-the-caribbean-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["419", "029"],
    members: [
      "ABW",
      "AIA",
      "ARG",
      "ATG",
      "BES",
      "BHS",
      "BLM",
      "BLZ",
      "BOL",
      "BRA",
      "BRB",
      "BVT",
      "CHL",
      "COL",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "ECU",
      "FLK",
      "GLP",
      "GRD",
      "GTM",
      "GUF",
      "GUY",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MAF",
      "MEX",
      "MSR",
      "MTQ",
      "NIC",
      "PAN",
      "PER",
      "PRI",
      "PRY",
      "SGS",
      "SLV",
      "SUR",
      "SXM",
      "TCA",
      "TTO",
      "URY",
      "VCT",
      "VEN",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "UNM49_MAF",
    name: "Middle Africa (UN M49)",
    slug: "middle-africa-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_3",
    translationCodes: ["017"],
    members: ["AGO", "CAF", "CMR", "COD", "COG", "GAB", "GNQ", "STP", "TCD"],
  },
  {
    code: "UNM49_MEL",
    name: "Melanesia (UN M49)",
    slug: "melanesia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["054"],
    members: ["FJI", "NCL", "PNG", "SLB", "VUT"],
  },
  {
    code: "UNM49_MIC",
    name: "Micronesia (UN M49)",
    slug: "micronesia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["057"],
    members: ["FSM", "GUM", "KIR", "MHL", "MNP", "NRU", "PLW", "UMI"],
  },
  {
    code: "UNM49_NAF",
    name: "Northern Africa (UN M49)",
    slug: "northern-africa-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["015"],
    members: ["DZA", "EGY", "ESH", "LBY", "MAR", "SDN", "TUN"],
  },
  {
    code: "UNM49_NAM",
    name: "Northern America (UN M49)",
    slug: "northern-america-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["003"],
    members: ["BMU", "CAN", "GRL", "SPM", "USA"],
  },
  {
    code: "UNM49_NEU",
    name: "Northern Europe (UN M49)",
    slug: "northern-europe-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["154"],
    members: [
      "ALA",
      "DNK",
      "EST",
      "FIN",
      "FRO",
      "GBR",
      "GGY",
      "IMN",
      "IRL",
      "ISL",
      "JEY",
      "LTU",
      "LVA",
      "NOR",
      "SJM",
      "SWE",
    ],
  },
  {
    code: "UNM49_OCE",
    name: "Oceania (UN M49)",
    slug: "oceania-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_1",
    translationCodes: ["009"],
    members: [
      "ASM",
      "AUS",
      "CCK",
      "COK",
      "CXR",
      "FJI",
      "FSM",
      "GUM",
      "HMD",
      "KIR",
      "MHL",
      "MNP",
      "NCL",
      "NFK",
      "NIU",
      "NRU",
      "NZL",
      "PCN",
      "PLW",
      "PNG",
      "PYF",
      "SLB",
      "TKL",
      "TON",
      "TUV",
      "UMI",
      "VUT",
      "WLF",
      "WSM",
    ],
  },
  {
    code: "UNM49_POL",
    name: "Polynesia (UN M49)",
    slug: "polynesia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["061"],
    members: ["ASM", "COK", "NIU", "PCN", "PYF", "TKL", "TON", "TUV", "WLF", "WSM"],
  },
  {
    code: "UNM49_SAF",
    name: "Southern Africa (UN M49)",
    slug: "southern-africa-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_3",
    translationCodes: ["018"],
    members: ["BWA", "LSO", "NAM", "SWZ", "ZAF"],
  },
  {
    code: "UNM49_SAM",
    name: "South America (UN M49)",
    slug: "south-america-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_3",
    translationCodes: ["005"],
    members: [
      "ARG",
      "BOL",
      "BRA",
      "BVT",
      "CHL",
      "COL",
      "ECU",
      "FLK",
      "GUF",
      "GUY",
      "PER",
      "PRY",
      "SGS",
      "SUR",
      "URY",
      "VEN",
    ],
  },
  {
    code: "UNM49_SAS",
    name: "Southern Asia (UN M49)",
    slug: "southern-asia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["034"],
    members: ["AFG", "BGD", "BTN", "IND", "IRN", "LKA", "MDV", "NPL", "PAK"],
  },
  {
    code: "UNM49_SEA",
    name: "South-eastern Asia (UN M49)",
    slug: "south-eastern-asia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["035"],
    members: ["BRN", "IDN", "KHM", "LAO", "MMR", "MYS", "PHL", "SGP", "THA", "TLS", "VNM"],
  },
  {
    code: "UNM49_SEU",
    name: "Southern Europe (UN M49)",
    slug: "southern-europe-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["039"],
    members: [
      "ALB",
      "AND",
      "BIH",
      "ESP",
      "GIB",
      "GRC",
      "HRV",
      "ITA",
      "MKD",
      "MLT",
      "MNE",
      "PRT",
      "SMR",
      "SRB",
      "SVN",
      "VAT",
    ],
  },
  {
    code: "UNM49_SSA",
    name: "Sub-Saharan Africa (UN M49)",
    slug: "sub-saharan-africa-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["202"],
    members: [
      "AGO",
      "ATF",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "DJI",
      "ERI",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "IOT",
      "KEN",
      "LBR",
      "LSO",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "MYT",
      "NAM",
      "NER",
      "NGA",
      "REU",
      "RWA",
      "SEN",
      "SHN",
      "SLE",
      "SOM",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "UNM49_WAF",
    name: "Western Africa (UN M49)",
    slug: "western-africa-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_3",
    translationCodes: ["011"],
    members: [
      "BEN",
      "BFA",
      "CIV",
      "CPV",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "LBR",
      "MLI",
      "MRT",
      "NER",
      "NGA",
      "SEN",
      "SHN",
      "SLE",
      "TGO",
    ],
  },
  {
    code: "UNM49_WAS",
    name: "Western Asia (UN M49)",
    slug: "western-asia-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["145"],
    members: [
      "ARE",
      "ARM",
      "AZE",
      "BHR",
      "CYP",
      "GEO",
      "IRQ",
      "ISR",
      "JOR",
      "KWT",
      "LBN",
      "OMN",
      "QAT",
      "SAU",
      "SYR",
      "TUR",
      "YEM",
    ],
  },
  {
    code: "UNM49_WEU",
    name: "Western Europe (UN M49)",
    slug: "western-europe-un-m49",
    regionType: "aggregate",
    definedBy: "un_m49_2",
    translationCodes: ["155"],
    members: ["AUT", "BEL", "CHE", "DEU", "FRA", "LIE", "LUX", "MCO", "NLD"],
  },
  {
    code: "UNSDG_ANZ",
    name: "Australia and New Zealand (UN SDG)",
    slug: "australia-and-new-zealand-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: ["AUS", "CCK", "CXR", "HMD", "NFK", "NZL"],
  },
  {
    code: "UNSDG_CSA",
    name: "Central and Southern Asia (UN SDG)",
    slug: "central-and-southern-asia-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: [
      "AFG",
      "BGD",
      "BTN",
      "IND",
      "IRN",
      "KAZ",
      "KGZ",
      "LKA",
      "MDV",
      "NPL",
      "PAK",
      "TJK",
      "TKM",
      "UZB",
    ],
  },
  {
    code: "UNSDG_ENA",
    name: "Europe and Northern America (UN SDG)",
    slug: "europe-and-northern-america-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: [
      "ALA",
      "ALB",
      "AND",
      "AUT",
      "BEL",
      "BGR",
      "BIH",
      "BLR",
      "BMU",
      "CAN",
      "CHE",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "FRO",
      "GBR",
      "GGY",
      "GIB",
      "GRC",
      "GRL",
      "HRV",
      "HUN",
      "IMN",
      "IRL",
      "ISL",
      "ITA",
      "JEY",
      "LIE",
      "LTU",
      "LUX",
      "LVA",
      "MCO",
      "MDA",
      "MKD",
      "MLT",
      "MNE",
      "NLD",
      "NOR",
      "POL",
      "PRT",
      "ROU",
      "RUS",
      "SJM",
      "SMR",
      "SPM",
      "SRB",
      "SVK",
      "SVN",
      "SWE",
      "UKR",
      "USA",
      "VAT",
    ],
  },
  {
    code: "UNSDG_ESEA",
    name: "Eastern and South-Eastern Asia (UN SDG)",
    slug: "eastern-and-south-eastern-asia-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: [
      "BRN",
      "CHN",
      "HKG",
      "IDN",
      "JPN",
      "KHM",
      "KOR",
      "LAO",
      "MAC",
      "MMR",
      "MNG",
      "MYS",
      "PHL",
      "PRK",
      "SGP",
      "THA",
      "TLS",
      "VNM",
    ],
  },
  {
    code: "UNSDG_LAC",
    name: "Latin America and the Caribbean (UN SDG)",
    slug: "latin-america-and-the-caribbean-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: [
      "ABW",
      "AIA",
      "ARG",
      "ATG",
      "BES",
      "BHS",
      "BLM",
      "BLZ",
      "BOL",
      "BRA",
      "BRB",
      "BVT",
      "CHL",
      "COL",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "ECU",
      "FLK",
      "GLP",
      "GRD",
      "GTM",
      "GUF",
      "GUY",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MAF",
      "MEX",
      "MSR",
      "MTQ",
      "NIC",
      "PAN",
      "PER",
      "PRI",
      "PRY",
      "SGS",
      "SLV",
      "SUR",
      "SXM",
      "TCA",
      "TTO",
      "URY",
      "VCT",
      "VEN",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "UNSDG_NAWA",
    name: "Northern Africa and Western Asia (UN SDG)",
    slug: "northern-africa-and-western-asia-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: [
      "ARE",
      "ARM",
      "AZE",
      "BHR",
      "CYP",
      "DZA",
      "EGY",
      "ESH",
      "GEO",
      "IRQ",
      "ISR",
      "JOR",
      "KWT",
      "LBN",
      "LBY",
      "MAR",
      "OMN",
      "PSE",
      "QAT",
      "SAU",
      "SDN",
      "SYR",
      "TUN",
      "TUR",
      "YEM",
    ],
  },
  {
    code: "UNSDG_OC",
    name: "Oceania (UN SDG)",
    slug: "oceania-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: [
      "ASM",
      "COK",
      "FJI",
      "FSM",
      "GUM",
      "KIR",
      "MHL",
      "MNP",
      "NCL",
      "NIU",
      "NRU",
      "PCN",
      "PLW",
      "PNG",
      "PYF",
      "SLB",
      "TKL",
      "TON",
      "TUV",
      "UMI",
      "VUT",
      "WLF",
      "WSM",
    ],
  },
  {
    code: "UNSDG_SSA",
    name: "Sub-Saharan Africa (UN SDG)",
    slug: "sub-saharan-africa-un-sdg",
    regionType: "aggregate",
    definedBy: "unsdg",
    members: [
      "AGO",
      "ATF",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "DJI",
      "ERI",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "IOT",
      "KEN",
      "LBR",
      "LSO",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "MYT",
      "NAM",
      "NER",
      "NGA",
      "REU",
      "RWA",
      "SEN",
      "SHN",
      "SLE",
      "SOM",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "UN_AFR",
    name: "Africa (UN)",
    slug: "africa-un",
    regionType: "aggregate",
    definedBy: "un",
    translationCodes: ["002"],
    members: [
      "AGO",
      "ATF",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "DJI",
      "DZA",
      "EGY",
      "ERI",
      "ESH",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "IOT",
      "KEN",
      "LBR",
      "LBY",
      "LSO",
      "MAR",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "MYT",
      "NAM",
      "NER",
      "NGA",
      "REU",
      "RWA",
      "SDN",
      "SEN",
      "SHN",
      "SLE",
      "SOM",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TUN",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "UN_ASI",
    name: "Asia (UN)",
    slug: "asia-un",
    regionType: "aggregate",
    definedBy: "un",
    translationCodes: ["142"],
    members: [
      "AFG",
      "ARE",
      "ARM",
      "AZE",
      "BGD",
      "BHR",
      "BRN",
      "BTN",
      "CHN",
      "CYP",
      "GEO",
      "HKG",
      "IDN",
      "IND",
      "IRN",
      "IRQ",
      "ISR",
      "JOR",
      "JPN",
      "KAZ",
      "KGZ",
      "KHM",
      "KOR",
      "KWT",
      "LAO",
      "LBN",
      "LKA",
      "MAC",
      "MDV",
      "MMR",
      "MNG",
      "MYS",
      "NPL",
      "OMN",
      "PAK",
      "PHL",
      "PRK",
      "QAT",
      "SAU",
      "SGP",
      "SYR",
      "THA",
      "TJK",
      "TKM",
      "TLS",
      "TUR",
      "UZB",
      "VNM",
      "YEM",
    ],
  },
  {
    code: "UN_EUR",
    name: "Europe (UN)",
    slug: "europe-un",
    regionType: "aggregate",
    definedBy: "un",
    translationCodes: ["150"],
    members: [
      "ALA",
      "ALB",
      "AND",
      "AUT",
      "BEL",
      "BGR",
      "BIH",
      "BLR",
      "CHE",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "FRO",
      "GBR",
      "GGY",
      "GIB",
      "GRC",
      "HRV",
      "HUN",
      "IMN",
      "IRL",
      "ISL",
      "ITA",
      "JEY",
      "LIE",
      "LTU",
      "LUX",
      "LVA",
      "MCO",
      "MDA",
      "MKD",
      "MLT",
      "MNE",
      "NLD",
      "NOR",
      "POL",
      "PRT",
      "ROU",
      "RUS",
      "SJM",
      "SMR",
      "SRB",
      "SVK",
      "SVN",
      "SWE",
      "UKR",
      "VAT",
    ],
  },
  {
    code: "UN_LAC",
    name: "Latin America and the Caribbean (UN)",
    slug: "latin-america-and-the-caribbean-un",
    regionType: "aggregate",
    definedBy: "un",
    translationCodes: ["419", "029"],
    members: [
      "ABW",
      "AIA",
      "ARG",
      "ATG",
      "BES",
      "BHS",
      "BLM",
      "BLZ",
      "BOL",
      "BRA",
      "BRB",
      "BVT",
      "CHL",
      "COL",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "ECU",
      "FLK",
      "GLP",
      "GRD",
      "GTM",
      "GUF",
      "GUY",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MAF",
      "MEX",
      "MSR",
      "MTQ",
      "NIC",
      "PAN",
      "PER",
      "PRI",
      "PRY",
      "SGS",
      "SLV",
      "SUR",
      "SXM",
      "TCA",
      "TTO",
      "URY",
      "VCT",
      "VEN",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "UN_NAM",
    name: "Northern America (UN)",
    slug: "northern-america-un",
    regionType: "aggregate",
    definedBy: "un",
    translationCodes: ["003"],
    members: ["BMU", "CAN", "GRL", "SPM", "USA"],
  },
  {
    code: "UN_OCE",
    name: "Oceania (UN)",
    slug: "oceania-un",
    regionType: "aggregate",
    definedBy: "un",
    translationCodes: ["009"],
    members: [
      "ASM",
      "AUS",
      "CCK",
      "COK",
      "CXR",
      "FJI",
      "FSM",
      "GUM",
      "HMD",
      "KIR",
      "MHL",
      "MNP",
      "NCL",
      "NFK",
      "NIU",
      "NRU",
      "NZL",
      "PCN",
      "PLW",
      "PNG",
      "PYF",
      "SLB",
      "TKL",
      "TON",
      "TUV",
      "UMI",
      "VUT",
      "WLF",
      "WSM",
    ],
  },
  {
    code: "URY",
    shortCode: "UY",
    name: "Uruguay",
    slug: "uruguay",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "USA",
    shortCode: "US",
    name: "United States",
    slug: "united-states",
    regionType: "country",
    isMappable: true,
    variantNames: ["US", "USA"],
  },
  {
    code: "UZB",
    shortCode: "UZ",
    name: "Uzbekistan",
    slug: "uzbekistan",
    regionType: "country",
    isMappable: true,
  },
  { code: "VAT", shortCode: "VA", name: "Vatican", slug: "vatican", regionType: "country" },
  {
    code: "VCT",
    shortCode: "VC",
    name: "Saint Vincent and the Grenadines",
    slug: "saint-vincent-and-the-grenadines",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "VEN",
    shortCode: "VE",
    name: "Venezuela",
    slug: "venezuela",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "VGB",
    shortCode: "VG",
    name: "British Virgin Islands",
    slug: "british-virgin-islands",
    regionType: "country",
  },
  {
    code: "VIR",
    shortCode: "VI",
    name: "United States Virgin Islands",
    slug: "united-states-virgin-islands",
    regionType: "country",
  },
  {
    code: "VNM",
    shortCode: "VN",
    name: "Vietnam",
    slug: "vietnam",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "VUT",
    shortCode: "VU",
    name: "Vanuatu",
    slug: "vanuatu",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "WB_EAP",
    name: "East Asia and Pacific (WB)",
    slug: "east-asia-and-pacific-wb",
    regionType: "aggregate",
    definedBy: "wb",
    translationCodes: ["030", "009"],
    members: [
      "ASM",
      "AUS",
      "BRN",
      "CCK",
      "CHN",
      "COK",
      "CXR",
      "FJI",
      "FSM",
      "GUM",
      "HKG",
      "IDN",
      "JPN",
      "KHM",
      "KIR",
      "KOR",
      "LAO",
      "MAC",
      "MHL",
      "MMR",
      "MNG",
      "MNP",
      "MYS",
      "NCL",
      "NFK",
      "NIU",
      "NRU",
      "NZL",
      "OWID_DRV",
      "OWID_KRU",
      "OWID_RVN",
      "PCN",
      "PHL",
      "PLW",
      "PNG",
      "PRK",
      "PYF",
      "SGP",
      "SLB",
      "THA",
      "TKL",
      "TLS",
      "TON",
      "TUV",
      "TWN",
      "UMI",
      "VNM",
      "VUT",
      "WLF",
      "WSM",
    ],
  },
  {
    code: "WB_ECA",
    name: "Europe and Central Asia (WB)",
    slug: "europe-and-central-asia-wb",
    regionType: "aggregate",
    definedBy: "wb",
    translationCodes: ["143", "150"],
    members: [
      "AIA",
      "ALA",
      "ALB",
      "AND",
      "ANT",
      "ARM",
      "AUT",
      "AZE",
      "BEL",
      "BES",
      "BGR",
      "BIH",
      "BLM",
      "BLR",
      "CHE",
      "CYP",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "FRO",
      "GBR",
      "GEO",
      "GGY",
      "GIB",
      "GLP",
      "GRC",
      "GRL",
      "GUF",
      "HRV",
      "HUN",
      "IMN",
      "IRL",
      "ISL",
      "ITA",
      "JEY",
      "KAZ",
      "KGZ",
      "LIE",
      "LTU",
      "LUX",
      "LVA",
      "MCO",
      "MDA",
      "MKD",
      "MNE",
      "MSR",
      "MTQ",
      "MYT",
      "NLD",
      "NOR",
      "OWID_AUH",
      "OWID_BAD",
      "OWID_BAV",
      "OWID_CYN",
      "OWID_CZS",
      "OWID_DSI",
      "OWID_GDR",
      "OWID_GFR",
      "OWID_KOS",
      "OWID_MOD",
      "OWID_PAR",
      "OWID_SAR",
      "OWID_SAX",
      "OWID_SEK",
      "OWID_SRM",
      "OWID_TSC",
      "OWID_USS",
      "OWID_WTB",
      "OWID_YGS",
      "POL",
      "PRT",
      "REU",
      "ROU",
      "RUS",
      "SHN",
      "SMR",
      "SRB",
      "SVK",
      "SVN",
      "SWE",
      "TJK",
      "TKM",
      "TUR",
      "UKR",
      "UZB",
      "VAT",
    ],
  },
  {
    code: "WB_LAC",
    name: "Latin America and Caribbean (WB)",
    slug: "latin-america-and-caribbean-wb",
    regionType: "aggregate",
    definedBy: "wb",
    translationCodes: ["419", "029"],
    members: [
      "ABW",
      "ARG",
      "ATG",
      "BHS",
      "BLZ",
      "BOL",
      "BRA",
      "BRB",
      "CHL",
      "COL",
      "CRI",
      "CUB",
      "CUW",
      "CYM",
      "DMA",
      "DOM",
      "ECU",
      "FLK",
      "GRD",
      "GTM",
      "GUY",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MAF",
      "MEX",
      "NIC",
      "OWID_GCL",
      "OWID_RCA",
      "PAN",
      "PER",
      "PRI",
      "PRY",
      "SGS",
      "SLV",
      "SUR",
      "SXM",
      "TCA",
      "TTO",
      "URY",
      "VCT",
      "VEN",
      "VGB",
      "VIR",
    ],
  },
  {
    code: "WB_MENAP",
    name: "Middle East, North Africa, Afghanistan and Pakistan (WB)",
    shortName: "MENA, Afghanistan and Pakistan (WB)",
    slug: "middle-east-north-africa-afghanistan-and-pakistan-wb",
    regionType: "aggregate",
    definedBy: "wb",
    translationCodes: ["145", "015"],
    members: [
      "AFG",
      "ARE",
      "BHR",
      "DJI",
      "DZA",
      "EGY",
      "IRN",
      "IRQ",
      "ISR",
      "JOR",
      "KWT",
      "LBN",
      "LBY",
      "MAR",
      "MLT",
      "OMN",
      "OWID_FPK",
      "PAK",
      "PSE",
      "QAT",
      "SAU",
      "SYR",
      "TUN",
      "YEM",
    ],
  },
  {
    code: "WB_NA",
    name: "North America (WB)",
    slug: "north-america-wb",
    regionType: "aggregate",
    definedBy: "wb",
    translationCodes: ["003"],
    members: ["BMU", "CAN", "SPM", "USA"],
  },
  {
    code: "WB_SA",
    name: "South Asia (WB)",
    slug: "south-asia-wb",
    regionType: "aggregate",
    definedBy: "wb",
    translationCodes: ["034"],
    members: ["BGD", "BTN", "IND", "IOT", "LKA", "MDV", "NPL"],
  },
  {
    code: "WB_SSA",
    name: "Sub-Saharan Africa (WB)",
    slug: "sub-saharan-africa-wb",
    regionType: "aggregate",
    definedBy: "wb",
    translationCodes: ["202"],
    members: [
      "AGO",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "ERI",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "KEN",
      "LBR",
      "LSO",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "NAM",
      "NER",
      "NGA",
      "OWID_ERE",
      "OWID_OFS",
      "RWA",
      "SDN",
      "SEN",
      "SLE",
      "SOM",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "WHO_AFR",
    name: "Africa (WHO)",
    slug: "africa-who",
    regionType: "aggregate",
    definedBy: "who",
    translationCodes: ["002"],
    members: [
      "AGO",
      "BDI",
      "BEN",
      "BFA",
      "BWA",
      "CAF",
      "CIV",
      "CMR",
      "COD",
      "COG",
      "COM",
      "CPV",
      "DZA",
      "ERI",
      "ETH",
      "GAB",
      "GHA",
      "GIN",
      "GMB",
      "GNB",
      "GNQ",
      "KEN",
      "LBR",
      "LSO",
      "MDG",
      "MLI",
      "MOZ",
      "MRT",
      "MUS",
      "MWI",
      "NAM",
      "NER",
      "NGA",
      "RWA",
      "SEN",
      "SLE",
      "SSD",
      "STP",
      "SWZ",
      "SYC",
      "TCD",
      "TGO",
      "TZA",
      "UGA",
      "ZAF",
      "ZMB",
      "ZWE",
    ],
  },
  {
    code: "WHO_AMR",
    name: "Americas (WHO)",
    slug: "americas-who",
    regionType: "aggregate",
    definedBy: "who",
    translationCodes: ["019"],
    members: [
      "AIA",
      "ARG",
      "ATG",
      "BHS",
      "BLZ",
      "BOL",
      "BRA",
      "BRB",
      "CAN",
      "CHL",
      "COL",
      "CRI",
      "CUB",
      "CYM",
      "DMA",
      "DOM",
      "ECU",
      "GRD",
      "GTM",
      "GUF",
      "GUY",
      "HND",
      "HTI",
      "JAM",
      "KNA",
      "LCA",
      "MEX",
      "MSR",
      "NIC",
      "PAN",
      "PER",
      "PRI",
      "PRY",
      "SLV",
      "SUR",
      "TTO",
      "URY",
      "USA",
      "VEN",
    ],
  },
  {
    code: "WHO_EMR",
    name: "Eastern Mediterranean (WHO)",
    slug: "eastern-mediterranean-who",
    regionType: "aggregate",
    definedBy: "who",
    translationCodes: ["145"],
    members: [
      "AFG",
      "ARE",
      "BHR",
      "DJI",
      "EGY",
      "IRN",
      "IRQ",
      "JOR",
      "KWT",
      "LBN",
      "LBY",
      "MAR",
      "OMN",
      "PAK",
      "QAT",
      "SAU",
      "SDN",
      "SOM",
      "SYR",
      "TUN",
      "YEM",
    ],
  },
  {
    code: "WHO_EUR",
    name: "Europe (WHO)",
    slug: "europe-who",
    regionType: "aggregate",
    definedBy: "who",
    translationCodes: ["150"],
    members: [
      "ALB",
      "AND",
      "ARM",
      "AUT",
      "AZE",
      "BEL",
      "BGR",
      "BIH",
      "BLR",
      "CHE",
      "CYP",
      "CZE",
      "DEU",
      "DNK",
      "ESP",
      "EST",
      "FIN",
      "FRA",
      "GBR",
      "GEO",
      "GRC",
      "HRV",
      "HUN",
      "IRL",
      "ISL",
      "ISR",
      "ITA",
      "KAZ",
      "KGZ",
      "LTU",
      "LUX",
      "LVA",
      "MCO",
      "MDA",
      "MKD",
      "MLT",
      "MNE",
      "NLD",
      "NOR",
      "POL",
      "PRT",
      "ROU",
      "RUS",
      "SMR",
      "SRB",
      "SVK",
      "SVN",
      "SWE",
      "TJK",
      "TKM",
      "TUR",
      "UKR",
      "UZB",
    ],
  },
  {
    code: "WHO_SEAR",
    name: "South-East Asia (WHO)",
    slug: "south-east-asia-who",
    regionType: "aggregate",
    definedBy: "who",
    translationCodes: ["035"],
    members: ["BGD", "BTN", "IDN", "IND", "LKA", "MDV", "MMR", "NPL", "PRK", "THA", "TLS"],
  },
  {
    code: "WHO_WPAC",
    name: "Western Pacific (WHO)",
    slug: "western-pacific-who",
    regionType: "aggregate",
    definedBy: "who",
    translationCodes: ["030", "035", "009"],
    members: [
      "ASM",
      "AUS",
      "BRN",
      "CHN",
      "COK",
      "FJI",
      "FSM",
      "GUM",
      "HKG",
      "JPN",
      "KHM",
      "KIR",
      "KOR",
      "LAO",
      "MAC",
      "MHL",
      "MNG",
      "MNP",
      "MYS",
      "NCL",
      "NIU",
      "NRU",
      "NZL",
      "PHL",
      "PLW",
      "PNG",
      "PYF",
      "SGP",
      "SLB",
      "TON",
      "TUV",
      "VNM",
      "VUT",
      "WSM",
    ],
  },
  {
    code: "WLF",
    shortCode: "WF",
    name: "Wallis and Futuna",
    slug: "wallis-and-futuna",
    regionType: "country",
    isUnlisted: true,
  },
  {
    code: "WSM",
    shortCode: "WS",
    name: "Samoa",
    slug: "samoa",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "YEM",
    shortCode: "YE",
    name: "Yemen",
    slug: "yemen",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ZAF",
    shortCode: "ZA",
    name: "South Africa",
    slug: "south-africa",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ZMB",
    shortCode: "ZM",
    name: "Zambia",
    slug: "zambia",
    regionType: "country",
    isMappable: true,
  },
  {
    code: "ZWE",
    shortCode: "ZW",
    name: "Zimbabwe",
    slug: "zimbabwe",
    regionType: "country",
    isMappable: true,
  },
] as const
