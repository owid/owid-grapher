...
export async function parseCSV(csv: string): Promise<string[][]> {
    return new Promise<string[][]>((resolve, reject) => {
        parse(csv, { relax_column_count: true, skip_empty_lines: true, trim: true },
            (err, rows) => {
                if (err)
                    reject(err)
                else
...
export class CSVStreamParser {
    ...
    constructor(input: ReadStream) {
        ...
        parser.on('readable', () => {
            this.isReadable = true; this.update()
        })
        parser.on('error', (err: any) => { this.error = err; this.update() })
        parser.on('end', () => { this.isEnded = true; this.update() })
