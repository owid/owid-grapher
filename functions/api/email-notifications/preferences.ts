...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
    try {
        ...
        if (data.action === "unsubscribeAll") {
            await unsubscribeFromMailchimp(env, user.email)
        } else if (data.subscribeToOwidBrief === false) {
            ...
            try {
                ...
                if (error instanceof MailchimpCleanedContactError) {
                    throw new JsonError(
                        "We couldn't resubscribe you to The OWID Brief: Mailchimp marked this address as undeliverable after earlier emails to it bounced. Please contact us at info@ourworldindata.org if you'd like it reinstated. Untick The OWID Brief to save your other changes.",
                        422
                    )
                }
                throw error
        ...
        if (
            data.action === "unsubscribeAll" ||
            !data.subscribeToTopicNotifications
        ) {
            await db
                .prepare(
                    `UPDATE users
