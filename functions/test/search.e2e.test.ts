...
// code under Node. This is what actually proves the Typesense client fetch
// calls work in the runtime they're deployed to.
//
// Requires a local Typesense with indexed data (`make up.headless` +
// `make reindex.typesense`), so it's gated behind the same env flag as
// searchApi.integration.test.ts instead of running unconditionally in CI.
