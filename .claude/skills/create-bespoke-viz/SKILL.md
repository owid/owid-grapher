...
For the composition, the standard is two stacked cards: a `Controls` box (gated on `!config.hideControls`) above a `Frame` holding `ChartHeader` → chart area → `ChartFooter`. Inside the controls box, dropdowns/switchers/checkboxes go in a `ControlsRow` and the `TimeSlider` sits outside the row. A project with a metadata file wraps that `Frame` in `BespokeMetadataProvider`, which adds a "Learn more about this data" link to the footer and puts the `MetadataModal` inside the frame. Pass it `isModalEnabled={!config.hideMetadataModal}` so a page showing the same metadata itself can switch the modal off.
