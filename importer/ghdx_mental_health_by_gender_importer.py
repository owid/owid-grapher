...
with transaction.atomic():
    ...
    existing_variables = Variable.objects.filter(datasetId__namespace='gbd_mental_health').values('name')
                ...
                if row['sex_name'] in sex_names and row['age_name'] in age_names and row['metric_name'] in metric_names and row['measure_name'] in measure_names:
                    if row['cause_name'] not in existing_subcategories_list:
                        ...
                        newdataset = Dataset(name=row['cause_name'],
                                             ...
                                             namespace='gbd_mental_health', categoryId=the_category,
...
newimport = ImportHistory(import_type='gbd_mental_health', import_time=timezone.now().strftime('%Y-%m-%d %H:%M:%S'),
                                  ...
                                  import_state='There are a total of %s gbd_mental_health variables after the import' % Variable.objects.filter(datasetId__namespace='gbd_mental_health').count())
newimport.save()
