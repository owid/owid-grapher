    ...
    if not import_history:
        ...
        for category in hnpqstats_categories_list:
            newdataset = Dataset(name='World Bank Health Nutrition and Population Statistics by Wealth Quintile - ' + category,
                                 ...
                                 namespace='hnpqstats', fk_dst_cat_id=the_category,
        ...
        available_variables = Variable.objects.filter(fk_dst_id__in=Dataset.objects.filter(namespace='hnpqstats'))
        ...
        for category in hnpqstats_categories_list:
            if category in cats_to_add:
                ...
                newdataset = Dataset.objects.get(name='World Bank Health Nutrition and Population Statistics by Wealth Quintile - ' + category, fk_dst_cat_id=DatasetCategory.objects.get(
                            ...
                            if len(data_values):
                                if indicator_code in category_vars[category]:
                                    ...
                                    if indicator_code in vars_to_add:
                                        ...
                                        if not global_cat[indicator_code]['saved']:
                                            newsource = Source.objects.get(name='World Bank Health Nutrition and Population Statistics by Wealth Quintile: ' + Variable.objects.get(code=indicator_code, fk_dst_id__in=Dataset.objects.filter(namespace='hnpqstats')).name)
                                            ...
                                            s_unit = short_unit_extract(global_cat[indicator_code]['unitofmeasure'])
                                            newvariable = Variable.objects.get(code=indicator_code, fk_dst_id__in=Dataset.objects.filter(namespace='hnpqstats'))
