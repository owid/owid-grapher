    ...
    if not import_history:
        ...
        for each in vars_to_delete:
            if each not in vars_being_used:
                ...
                source_object = Variable.objects.get(code=each, fk_dst_id__in=Dataset.objects.filter(namespace='findex')).sourceId
                Variable.objects.get(code=each, fk_dst_id__in=Dataset.objects.filter(namespace='findex')).delete()
        ...
        all_findex_datasets_with_vars = Variable.objects.filter(fk_dst_id__in=all_findex_datasets).values(
            'fk_dst_id').distinct()
        all_findex_datasets_with_vars_dict = {item['fk_dst_id'] for item in all_findex_datasets_with_vars}
