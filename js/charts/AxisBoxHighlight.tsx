...
export default class AxisBoxHighlight extends React.Component<{ axisBox: AxisBox, value: { x: number, y: number }}> {
    render() {
        const {axisBox, value} = this.props
        const {xScale, yScale, bounds, innerBounds, xAxis, yAxis} = axisBox

        const xLabel = new TextWrap({ maxWidth: bounds.width, fontSize: 0.7, text: xScale.tickFormat(value.x) })
        const yLabel = new TextWrap({ maxWidth: bounds.width, fontSize: 0.7, text: yScale.tickFormat(value.y) })
        const highlightX = xScale.place(value.x)
        const highlightY = yScale.place(value.y)
        const xLabelBounds = new Bounds(highlightX-xLabel.width/2, bounds.bottom-xAxis.labelOffset-xLabel.height, xLabel.width, xLabel.height)
