...
export class VerticalAxis {
    ...
    @computed get label(): TextWrap | undefined {
        ...
        return props.labelText ? new TextWrap({ maxWidth: height, fontSize: 0.7*props.fontSize, text: props.labelText }) : undefined
    }
    ...
    @computed get width() {
        ...
        return Bounds.forText(longestTick, { fontSize: this.tickFontSize }).width + labelOffset + 5
    }
...
export class VerticalAxisView extends React.Component<{ bounds: Bounds, axis: VerticalAxis, onScaleTypeChange?: (scale: ScaleType) => void }> {
    render() {
        ...
        return <g className="VerticalAxis">
            ...
            {ticks.map((tick, i) =>
                <text key={i} x={(bounds.left + axis.width - 5).toFixed(2)} y={scale.place(tick)} fill={textColor} dominantBaseline="middle" textAnchor="end" fontSize={axis.tickFontSize}>{scale.tickFormat(tick)}</text>
            )}
