    ...
    render() {
        ...
        return <g className="AxisGridLines">
            {_.map(scale.getTickValues(), v => {
                if (orient == 'left')
                    return <line x1={bounds.left} y1={Math.round(scale.place(v))} x2={bounds.right} y2={Math.round(scale.place(v))} stroke={v == 0 ? "#aaa" : "#eee"} stroke-dasharray={v != 0 && "3,2"}/>
                else
                    return <line x1={Math.round(scale.place(v))} y1={bounds.bottom} x2={Math.round(scale.place(v))} y2={bounds.top} stroke={v == 0 ? "#aaa" : "#eee"} stroke-dasharray={v != 0 && "3,2"}/>
