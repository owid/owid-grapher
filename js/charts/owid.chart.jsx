...
export default function() {
    ...
    chart.flow('tabs : model', function() {
        return {
            ...
            data: DataTab(chart),
            map: { name: 'map', isOverlay: false },
            sources: { name: 'sources', isOverlay: true },
    ...
    chart.flow('overlayTab : tabs, activeTabName', function(tabs, activeTabName) {
        var tab = tabs[activeTabName];
        if (chart.overlayTab && chart.overlayTab.clean) chart.overlayTab.clean();
    ...
    let primaryTabInstance = null
    let overlayTabInstance = null
    ...
    chart.render = function() {
        requestAnimationFrame(function() {
            chart.now('el, header, primaryTab, overlayTab, innerBounds, scale, loadingIcon, isEmbed', function(el, header, primaryTab, overlayTab, innerBounds, scale, loadingIcon, isEmbed) {
                ...
                if (primaryTab && primaryTab.name == 'map') {
                   chart.mapdata.update();

                   primaryTabInstance = render(<MapTab bounds={bounds} chartView={this} choroplethData={chart.mapdata.currentValues} years={chart.map.getYears()} inputYear={+chart.map.get('targetYear')} legendData={chart.mapdata.legendData} legendTitle={chart.mapdata.legendTitle} projection={chart.map.get('projection')} defaultFill={chart.mapdata.getNoDataColor()} />, chart.svg.node(), primaryTabInstance)
                } else {
                    primaryTabInstance = render("", chart.svgNode, primaryTabInstance)
                    if (primaryTab)
                       primaryTab.render(bounds);
                }

                if (overlayTab) {
                    if (overlayTab.name == 'sources') {
                        overlayTabInstance = render(<SourcesTab bounds={bounds.scale(chart.scale)} sources={chart.data.transformDataForSources()}/>, chart.htmlNode, overlayTabInstance)
                    } else {
                        overlayTabInstance = render("", chart.htmlNode, overlayTabInstance)
