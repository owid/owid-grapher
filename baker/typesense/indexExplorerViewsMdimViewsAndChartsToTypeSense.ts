...
/** Longer titles are never this long; see `computeRankTiebreaker`. */
const MAX_TITLE_LENGTH = 999

/**
 * Packs Algolia's last two `customRanking` criteria —
 * `asc(viewTitleIndexWithinExplorer)`, then `asc(titleLength)` — into a single
 * ascending integer, because Typesense permits only 3 `sort_by` fields and
 * `_text_match` plus `score` already take two.
 *
 * Sorting by this is identical to sorting by the pair as long as titleLength
 * stays below MAX_TITLE_LENGTH, hence the clamp.
 */
export function computeRankTiebreaker(record: {
    // Only explorer-view records carry this; charts and mdim views rank as 0,
    // exactly as Algolia treats a missing customRanking attribute.
    viewTitleIndexWithinExplorer?: number
    titleLength?: number
}): number {
    const viewIndex = record.viewTitleIndexWithinExplorer ?? 0
    const titleLength = Math.min(record.titleLength ?? 0, MAX_TITLE_LENGTH)
    return viewIndex * (MAX_TITLE_LENGTH + 1) + titleLength
}
...
const indexExplorerViewsMdimViewsAndChartsToTypeSense = async () => {
    ...
    const typeSenseRecords = records.map((record) => ({
        ...
        rankTiebreaker: computeRankTiebreaker(record),
    }))
