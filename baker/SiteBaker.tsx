...
import { GdocPostArchivalManifest } from "../serverUtils/archivalUtils.js"
...
export const bakeSingleGdocPostPageForArchival = async (
    bakedSiteDir: string,
    gdocSlug: string,
    knex: db.KnexReadonlyTransaction,
    {
        ...
        manifest: GdocPostArchivalManifest
    }
) => {
    const gdoc = await getAndLoadGdocBySlug(knex, gdocSlug)
    await gdoc.loadState(knex)

    const outPathHtml = `${bakedSiteDir}/${gdocSlug}.html`
    await fs.mkdirp(path.dirname(outPathHtml))
    await fs.writeFile(outPathHtml, renderGdoc(gdoc, false, archiveContext))

    const outPathManifest = `${bakedSiteDir}/${gdocSlug}.manifest.json`
