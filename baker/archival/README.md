...
flowchart TD
 ...
 subgraph runtime["Runtime assets"]
  ...
  subgraph other["Other files"]
    ...
    page ~~~ other
    explorer -- bakes into HTML --> static
    explorer -- loads at runtime --> runtime
    explorer ~~~ other
    article -- bakes into HTML --> static
    article -- loads at runtime --> runtime
    article ~~~ other
```
...
- `POSTID` is a post ID (Gdoc ID), e.g. `1g2h3i4j`
...
- Any interactive charts used in or linked from the article have changed (using the same logic as above)
...
6. Lastly, after the staging server build is completed, we serve the archive on port 8789.
...
- Interactive charts, images and videos are loaded from the archive (using the `runtimeAssetMap`).
