...
_Selection & canvas UX addendum (2026-07-02)_: block selection is now the single editing model — every block NodeView (atoms **and** layout containers, which became React NodeViews) shares a `BlockFrame`: a thin border that appears when hovering the block's edges, selects the block on click, and doubles as the drag handle for reordering; the per-block toolbars/cogwheel buttons are gone. Selecting any block (border or body click) opens its typed inspector in the right rail, derived from the ProseMirror `NodeSelection` (`adminSiteClient/richEditor/inspection.ts`) — this is the addressing model the future command layer (§9) reuses. The canvas now spans the full window (wide max 1280px, text blocks centered at the 680px article measure, mirroring the site grid) so two-column layouts and charts render at desktop proportions by default. Inserting from the palette while a block is selected asks whether to replace the selected block or insert below it.

_Addendum 2 (2026-07-03)_: blockquote, callout and aside became React NodeViews too, so every block type now has the same hover-border selection (previously only atoms and layout containers had it). **Table** and **pull-quote** were promoted from props atoms to real containers: tables are nested `tableBlock > tableRow > tableCell` nodes whose cells hold arbitrary editable blocks (typing and drag & drop into cells work; add/remove row/column commands live in the inspector), and a pull quote's attribution text blocks are an editable content hole while the quote/alignment stay inspector-edited. The corpus round-trip remains 1,173/1,173. The inspector now covers essentially all block settings: list editors for recirc links, all-charts pinned URLs and research-and-writing primary/secondary links; image mobile-filename; prominent-link thumbnail; narrative-chart size/height; the full four pull-quote alignments; blockquote citation; callout title/icon; aside position; table template/size/caption; R&W heading/hide-flags/variant. R&W row sections and the "more" list remain JSON-edited.
...
**M4 — AI side panel** (detailed plan: `rich-editing-m4-plan.md`; **built 2026-07-04**, see its §7b)

- In-browser agent on the pi harness (pi-ai / pi-agent-core / pi-web-ui, as in the [gdocs Chrome extension](https://github.com/owid/gdocs-chrome-extension)); tools adapted from the extension (outline/read/find/edit/selection/comments/describe_component/code-VFS/web/summaries + search_charts + write_doc_from_file); semantic XHTML (PR #6236 codec resurrected into db/model/Gdoc, id-attribute round-trip added, corpus 1,174/1,174) as the model-facing format; edits applied immediately through the live Yjs editor (no apply gate), one undo step per operation, verified under concurrent two-browser typing. Browser API keys v1 (proxy later), pi-web-ui ChatPanel embedded (Tailwind-v4 @layer makes scoping unnecessary), blockIds extended to all top-level blocks.
