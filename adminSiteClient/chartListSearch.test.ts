...
describe(filterChartsBySearchString, () => {
    ...
    it("supports fielded terms", () => {
        expect(
            filterChartsBySearchString(charts, "id:3").map((c) => c.id)
        ).toEqual([3])
        expect(
            filterChartsBySearchString(charts, "tag:Health").map((c) => c.id)
        ).toEqual([2])
        expect(
            filterChartsBySearchString(charts, "published:false").map(
                (c) => c.id
            )
        ).toEqual([1, 2, 3])
        expect(
            filterChartsBySearchString(charts, "by:Ada").map((c) => c.id)
        ).toEqual([3])
    })
