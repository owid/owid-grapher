...
> {
    ...
    // The two registry fetches are what the whole page is built from; when
    // they fail there is nothing to degrade to, so say so rather than sitting
    // on "Loading…" forever.
    ...
    private previewPathForComponent(
            ...
            <nav className="gdocs-ref__nav" aria-label="Writing reference">
                <div className="gdocs-ref__search">
                    ...
                    <input
                        ...
                        aria-label="Search the writing reference"
                    />
                </div>
                <Link
                    className={
    ...
    private renderComponentDetail(
        ...
        return (
            <article className="gdocs-ref__detail">
                ...
                {/* One route serves every component, so without a key the
                    forms section would carry its doc-type filter and expanded
                    long tail over to the next component. */}
                <ComponentForms
                    key={component.id}
                    component={component}
                    usage={this.usageOf(component)}
        ...
        if (this.registryFailed)
            return (
                <div className="gdocs-ref__empty">
                    The writing reference could not be loaded. Reload the page
                    to try again.
                </div>
            )
    ...
    async getData(): Promise<void> {
        ...
        // admin.getJSON already shows the error modal; catching keeps the
