 ...
 * material for the value chips. Multiline and nested values simply stay
 * unknown (the chip reads "set").
 */
...
/** One property row of a form card's Sets column. */
interface FormPropInfo {
    name: string
    /** Chip text: the authored value, "set", or "none" */
    baseDisplay: string
    baseSet: boolean
    required: boolean
    content: boolean
...
function buildRows(
    ...
    const rows = doc.props.map((prop): FormPropInfo => {
    ...
    // Set value props lead (they are what the form is), then the unset
    // value props, then the block-content scaffolding — stable within each
    // group.
    const rank = (row: FormPropInfo): number =>
        row.baseSet && !row.content ? 0 : !row.content ? 1 : 2
    return rows
...
/** One property row: name on the left, its value chip on the right. */
function FormPropRow({ row }: { row: FormPropInfo }): React.ReactElement {
    const display = row.baseSet ? row.baseDisplay : "none"
 ...
 * this form. Signature parts of props the registry does not declare (rare)
 * still render as static chips so no form ever hides a part.
 */
function FormProps({
    rows,
    extraParts,
    ...
    return (
        <div className="gdocs-ref-live__form-sets">
            ...
            <div className="gdocs-ref-live__form-props">
                {rows.map((row) => (
                    <FormPropRow key={row.name} row={row} />
                ))}
                {extraParts.map((part) => {
...
function FormCard({
    ...
    const rows = useMemo(
        () => buildRows(doc, variation, baseScalars, live),
        [doc, variation, baseScalars, live]
    )
    ...
    return (
        ...
        >
            ...
            <div className="gdocs-ref-live__form-card-body">
                <FormProps rows={rows} extraParts={extraParts} />
...
function ComponentProperties({
    ...
    const hasDescriptions = doc.props.some((prop) => prop.description)
