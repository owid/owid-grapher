...
export function GdocsReferenceMarkdown({
    ...
    // search does). The useMemo below therefore has no dependencies: every
    // prop it needs is reached through the ref.
    const latest = useRef({ examples, previewPathForExample, componentIds })
    latest.current = { examples, previewPathForExample, componentIds }

    const markdownComponents: MarkdownComponents = useMemo(
        () => ({
            pre: ({ children }) => {
                const { examples, previewPathForExample } = latest.current
            ...
            code: ({ children, ...props }) => {
                const { componentIds } = latest.current
