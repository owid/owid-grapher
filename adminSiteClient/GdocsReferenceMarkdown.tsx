 ...
 * recognise that shape. Only a component that exists links, though: property
 * descriptions also spell out nested tags (`{.config}`, `{.technical}`) that
 * are not components, and those stay plain code. A `{guide:id}` /
 * `{template:id}` mention instead links to the target's title, styled as a
 * small pill rather than code (falling back to the raw mention text when no
 * title resolves). Anything else stays plain code.
 */
function CodeSpan({
    text,
    titleFor,
    ...
    if (mention.kind === "component") {
        if (titleFor && titleFor(mention) === undefined)
