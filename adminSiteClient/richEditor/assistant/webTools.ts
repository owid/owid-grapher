...
export const geminiGroundedSearch = async (
    ...
    if (!resp.ok)
        throw new Error(
            `Gemini grounding failed: ${data.error?.message ?? `HTTP ${resp.status}`}`
        )
...
export const extractReadable = async (
    html: string,
    url: string
...
export const webSearchTool = (
    ...
    execute: async (
        ...
        AgentToolResult<{ results: number; searchSuggestionsHtml?: string }>
    > => {
        if (!host.web)
            throw new Error("Web search is not available in this environment.")
        const num = clamp(Math.round(params.num ?? 5), 1, 10)
        const query = params.query.trim()
        if (query === "")
            return {
                content: text("Empty query — provide search terms."),
                details: { results: 0 },
            }
        ...
        if (results.length === 0 && !answer)
            return {
                ...
                details: searchSuggestionsHtml
                    ? { results: 0, searchSuggestionsHtml }
                    : { results: 0 },
            }
...
export const readUrlTool = (
    host: WebToolHost
): AgentTool<typeof readUrlParams> => ({
    label: "Read URL",
    name: "read_url",
    ...
    > => {
        if (!host.web)
            throw new Error(
                "Reading web pages is not available in this environment."
            )
        ...
        if (blocked)
            return {
                ...
                details: { url, words: 0, truncated: false },
            }

        const maxChars = clamp(
            Math.round(params.max_chars ?? READ_URL_DEFAULT_CHARS),
            500,
            50000
        )
        ...
        if (res.status >= 400)
            return {
                content: text(
                    `The server returned HTTP ${res.status} for ${res.finalUrl}.`
                ),
