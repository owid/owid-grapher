...
async function fetchMultiDims(admin: Admin) {
    const result = await admin.getJSON<{ multiDims: ApiMultiDim[] }>(
        "/api/multi-dims.json"
    )
    return result.multiDims.map(deserializeMultiDim)
}

async function patchMultiDim(admin: Admin, id: number, data: Json) {
    const result = await admin.requestJSON<{ multiDim: ApiMultiDim }>(
        `/api/multi-dims/${id}`,
        data,
        "PATCH"
    )
    return deserializeMultiDim(result.multiDim)
}
