...
export class StaticVizTable1751646349658 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Add imageText column to images table for OCR text
        await queryRunner.query(`-- sql
            ALTER TABLE images ADD COLUMN imageText text DEFAULT NULL
        `)

        // Create static_viz table without imageText and mobileImageText columns
        await queryRunner.query(`-- sql
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`-- sql
            ALTER TABLE images DROP COLUMN IF EXISTS imageText;
        `)
    }
}
