...
select * from complete_redirects where targetDomain is not null and targetDomain <> '';
...
drop function extract_url_domain;
...
AFTER INSERT ON wordpress_redirects
FOR EACH ROW
INSERT INTO
  complete_redirects (
    wordpressRedirectId,
...
-- TODO: add trigger on charts to keep existing chart slug updates in sync with
-- the complete_redirects table and the same for chartId updates on chart_slug_redirects

 -- TODO: finish trigger for updating complete_redirects
CREATE TRIGGER
...
select extract_url_domain('http://ourworldindata.org/path/test?query=123&other=456#fragment')
select extract_url_path('http://ourworldindata.org/path/test?query=123&other=456#fragment')
select extract_url_query('http://ourworldindata.org/path/test?query=123&other=456#fragment')
select extract_url_fragment('http://ourworldindata.org/path/test?query=123&other=456#fragment')
select extract_url_domain('/path/test?query=123&other=456#fragment')
select extract_url_path('/path/test?query=123&other=456#fragment')
select extract_url_query('/path/test?query=123&other=456#fragment')
select extract_url_fragment('/path/test?query=123&other=456#fragment')
select extract_url_domain('https://test.com')
select extract_url_path('https://test.com')


select extract_url_domain('/')
select extract_url_path('/')
select extract_url_query('/')
select extract_url_fragment('/')



select extract_url_domain('/');

select extract_url_domain('/atom.xml')

select extract_url_path('/atom.xml')

select extract_url_query('/atom.xml')

select extract_url_fragment('/atom.xml')


-- DROP STATMENTS
-- DROP TABLE manual_bulk_redirects;

-- DROP TABLE wordpress_redirects;

-- DROP TABLE wordpress_redirects_candidates;

-- DROP TABLE complete_redirects;

-- DROP TABLE final_slugs;

-- DROP FUNCTION extract_url_domain;

-- DROP FUNCTION extract_url_path;

-- DROP FUNCTION extract_url_query;

-- DROP FUNCTION extract_url_fragment;

-- DROP TRIGGER chart_slug_redirects_insert_complete;

-- DROP TRIGGER chart_update_complete_redirects;

-- DROP TRIGGER chart_slug_redirects_update_complete;

-- DROP TRIGGER chart_slug_redirects_delete_complete;

-- DROP TRIGGER chart_insert_final_slugs;

-- DROP TRIGGER chart_update_final_slugs;

-- DROP TRIGGER chart_delete_final_slugs;

-- DROP TRIGGER posts_insert_final_slugs;

-- DROP TRIGGER posts_update_final_slugs;

-- DROP TRIGGER posts_delete_final_slugs;
