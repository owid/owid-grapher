...
- createdAt fields are generally autogenerated by MySQL if they are present, so they usually do not need to be set manually in insert calls
- updatedAt fields are usually maintained automatically by MySQL as well, but there are exceptions. In particular, `charts.updatedAt` and `chart_configs.updatedAt` are intentionally managed in application code so related updates can share the exact same timestamp across tables.
- Keep every field `description` under 1024 characters. owid/analytics mirrors these tables into BigQuery and stamps these descriptions onto the BigQuery columns (`ana docs mysql-stamp-docs`), and BigQuery rejects any column description longer than that. The table-level `metadata.description` has a much higher cap (16384), and longer prose belongs in this README or under `docs/`.
