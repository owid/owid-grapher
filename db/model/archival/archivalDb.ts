...
async function resolveMultiDimRedirects(
    ...
    for (const slug of slugs) {
        redirectMap.set(slug, redirects.get(slug)?.targetSlug ?? slug)
    }
...
async function resolveGrapherSlugsAndFetchChecksums(
    ...
    const slugsRedirectingToMultiDim = new Set(
        Array.from(slugsAfterChartRedirects).filter(
            (slug) => multiDimRedirects.get(slug) !== slug
        )
    )
...
async function resolveExplorerSlugsAndFetchChecksums(
    ...
    const slugsRedirectingToMultiDim = new Set(
        originalSlugs.filter((slug) => multiDimRedirects.get(slug) !== slug)
    )
