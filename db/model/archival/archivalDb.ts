...
export const getPostChecksumsFromDb = async (
    ...
    for (const [, charts] of chartsByPost) {
        charts.grapher = new Set(
            Array.from(charts.grapher).map(
                (slug) => grapherSlugRedirects.get(slug) ?? slug
            )
        )
    }

    const resolvedGrapherSlugs = new Set<string>()
    for (const [, charts] of chartsByPost) {
        charts.grapher.forEach((slug) => resolvedGrapherSlugs.add(slug))
    }
...
export const findChangedPostPages = async (
    ...
    const postIdsToArchive = new Set(
        needToBeArchived.map((post) => post.postId)
    )
