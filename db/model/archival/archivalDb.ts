...
export const getGrapherChecksumsFromDb = async (
    ...
    type FlatGrapherChecksums = {
        ...
        deprecationNoticeMd5: string | null
    ...
    const rows: GrapherChecksumsObjectWithHash[] = await db
        ...
        SELECT
            ...
            -- NULL, not MD5(""), when there is no notice: the key is then left out of the hashed
        ...
        .then((row) =>
            row.map((r) => ({
                ...
                checksums: {
                    ...
                    // Spread rather than assign null: safe-stable-stringify keeps null but drops
                    // undefined, and an absent key is what preserves the existing hashes.
                    ...(r.deprecationNoticeMd5
...
export const hashGrapherChecksumsObj = (
