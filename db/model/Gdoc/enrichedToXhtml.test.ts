...
describe(spanToXhtml, () => {
    ...
    describe("fallback span", () => {
        it("serializes as a fallback element so the span round-trips", () => {
            const span: Span = {
                spanType: "span-fallback",
                children: [{ spanType: "span-simple-text", text: "text" }],
            }
            expect(spanToXhtml(span)).toBe("<fallback>text</fallback>")
        })
    })
})
...
describe(enrichedBlockToXhtml, () => {
    ...
    describe("round-trip-only block regressions", () => {
        ...
        it("round-trips fallback spans inside text blocks", () => {
            const block: EnrichedBlockText = {
                type: "text",
                value: [
                    { spanType: "span-simple-text", text: "The index (" },
                    {
                        spanType: "span-fallback",
                        children: [
                            {
