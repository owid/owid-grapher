        ...
        return children
            .map((child, index, arr) => {
                if (child.type === "text") {
                    ...
                    // The result is parsed as HTML again by htmlToSpans, so
                    // literal markup characters in the text (e.g. "</>", "&")
                    // must be re-escaped or they would be eaten as tags.
                    return text
                        .replace(/&/g, "&amp;")
                        .replace(/</g, "&lt;")
                        .replace(/>/g, "&gt;")
 ...
 * Throws XhtmlParseError for unknown block types.
 */
// elementToRawBlock builds raw values with explicitly-undefined keys for
// absent attributes (e.g. { hasOutline: attribs.hasOutline }), and
// validateRawBoolean treats present-but-undefined as invalid ("set to
// 'undefined'"), so e.g. <image filename="x"/> would fail to parse. Strip
// undefined keys recursively before the raw→enriched validation.
