    ...
    if (span) {
        ...
        const result: BlockParseResult<ArchieBlockOrWpComponent> = match(
            ...
            .with({ tagName: "div" }, (element) => {
                ...
                // We're adding this special case here, because otherwise the heading element they (frequently) contain
                // would mess with the all-charts placement logic (grep "indexOfFirstHeading")
                if (className && className.includes("pcrm")) {
                ...
                return unwrapElementWithContext(element)
            })
