...
export const componentTypes = (): ComponentType[] =>
    (Object.keys(enrichedBlockExamples) as ComponentType[]).filter(
        (t) => !NON_AUTHORABLE.has(t)
    )
...
const parsesCleanly = (
    ...
    try {
        ...
        return (
            collectBlockErrors(back[0]).filter((e) => !e.startsWith("warning:"))
                .length === 0
        )
    } catch {
        return false
...
const probeFields = (type: ComponentType): ProbeResult => {
    const example = enrichedBlockExamples[type] as unknown as Record<
        string,
        unknown
    >
    const fields = Object.keys(example).filter(
        (k) => k !== "type" && k !== "parseErrors"
    )
