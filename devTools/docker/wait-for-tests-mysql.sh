...
while ! mysql -u$GRAPHER_TEST_DB_USER -p$GRAPHER_TEST_DB_PASS -h $GRAPHER_TEST_DB_HOST --port=$GRAPHER_TEST_DB_PORT -e 'select 1' $GRAPHER_TEST_DB_NAME &>/dev/null; do
    printf '.'
    sleep 1
done
printf 'ok\n'
