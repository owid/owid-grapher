...
describe("the sidecar section split", () => {
    ...
    // A free section is allowed to start with a common word. Only the
    // multi-word structural headings match on prefix.
    test.each(["## Notes on methodology", "## Properties of the source data"])(
        "lets %s through as free prose",
        (heading) => {
            const { prose } = split("Intro.\n\n" + heading + "\n\nSomething.")
            expect(prose.notes).toBe(heading + "\n\nSomething.")
        }
    )

    // Normalizes to "", which is a prefix of every canonical heading.
    test("does not blame a symbol-only heading on the first section", () => {
        const { prose } = split("Intro.\n\n## 🚀\n\nSomething.")
        expect(prose.notes).toBe("## 🚀\n\nSomething.")
    })
