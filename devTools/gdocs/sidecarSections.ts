...
function nearestSection(
    ...
    // A heading with no letters or digits ("## 🚀") normalizes to "", which
    // is a prefix of every canonical heading. It is not a misspelling of any
    // of them; let it through as free prose.
    if (!candidate) return undefined
    for (const section of SIDECAR_SECTIONS) {
        const canonical = normalizeHeading(section.heading)
        // Only the multi-word headings match on prefix. "When to use this
        // block" is unmistakably the decision section with filler appended,
        // but "Notes on methodology" and "Properties of the source" are
        // ordinary free sections that merely start with a common word —
        // matching those on prefix rejects them with a suggestion the author
        // did not want. Single-word headings rely on edit distance instead,
        // which still catches "Note" and "Propertes".
        if (
            section.heading.includes(" ") &&
            (candidate.startsWith(canonical) || canonical.startsWith(candidate))
        )
            return section
