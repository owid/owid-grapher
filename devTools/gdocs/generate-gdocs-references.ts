...
// staleness check fails on a file nobody meaningfully changed.
...
function extractComponentReferences(
    ...
    // Components that render no example at all. Not a build error: a handful
    // are still unwritten, and system/auto-generated blocks are never authored
    // by hand. Reported so an example run that harvests nothing stays visible
    // instead of shipping a reference page with nothing to copy.
    const exampleTodos: string[] = []
    for (const ident of memberIds) {
        ...
        if (examples.length === 0 && !fm.system && !fm.autoGenerated)
            exampleTodos.push(id)
    }
...
async function main(): Promise<void> {
    ...
    try {
        ...
        // Explicit locale: the default varies with the machine's ICU settings,
        // which would order the committed registry differently on CI than on
        // the machine that generated it, failing the staleness check.
        allComponents.sort((a, b) => a.title.localeCompare(b.title, "en"))
        console.log(
            "Extracted " +
        ...
        await fs.writeFile(JSON_OUT, toJsonFile(registry))
        console.log("Wrote " + path.relative(process.cwd(), JSON_OUT))
        await fs.writeFile(TEMPLATES_JSON_OUT, toJsonFile(templates))
        console.log("Wrote " + path.relative(process.cwd(), TEMPLATES_JSON_OUT))
        await fs.writeFile(GUIDES_JSON_OUT, toJsonFile(guides))
