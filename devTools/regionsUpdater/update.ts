...
const ETL_REGIONS_URL =
    ...
    GRAPHER_REGIONS_PATH = `${GRAPHER_ROOT}/packages/@ourworldindata/utils/src/regions/regions.data.ts`,
...
async function prettifiedTopology(geoJson: FeatureCollection): Promise<string> {
    ...
    const formatted = await prettifiedTs(
...
async function main() {
    ...
    // generate new regions.data.ts file and compare to old version
    const regionsTs = await prettifiedTs(
        GENERATED_FILE_HEADER +
            `export const regionsData = ${JSON.stringify(entities)} as const`
    )
    if (await didChange(GRAPHER_REGIONS_PATH, regionsTs)) {
        await writeFile(GRAPHER_REGIONS_PATH, regionsTs)
