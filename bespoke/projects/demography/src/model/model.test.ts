...
describe("Migration options state management", () => {
    afterEach(() => {
        setMigrationOptions(DEFAULT_MIGRATION_OPTIONS)
    })

    describe(setMigrationOptions, () => {
        it("merges partial options with current active options", () => {
            setMigrationOptions({ schedule: "low_dependency" })
            const options = getMigrationOptions()
            expect(options.schedule).toBe("low_dependency")
            expect(options.direction).toBe(DEFAULT_MIGRATION_OPTIONS.direction)
        })

        it("resets invalid schedule to default", () => {
            setMigrationOptions({ schedule: "invalid_schedule" })
            expect(getMigrationOptions().schedule).toBe("western_standard")
        })

        it("resets invalid direction to default", () => {
            setMigrationOptions({ direction: "invalid" })
            expect(getMigrationOptions().direction).toBe("auto")
        })

        it("resets invalid sexPattern to default", () => {
            setMigrationOptions({ sexPattern: "invalid" })
            expect(getMigrationOptions().sexPattern).toBe("neutral")
        })

        it("clamps flowTurnoverRate to non-negative", () => {
            setMigrationOptions({ flowTurnoverRate: -0.5 })
            expect(
                getMigrationOptions().flowTurnoverRate
            ).toBeGreaterThanOrEqual(0)
        })

        it("clamps externalSexShareBlend to [0, 1]", () => {
            setMigrationOptions({ externalSexShareBlend: 2.0 })
            expect(
                getMigrationOptions().externalSexShareBlend
            ).toBeLessThanOrEqual(1)

            setMigrationOptions({ externalSexShareBlend: -0.5 })
            expect(
                getMigrationOptions().externalSexShareBlend
            ).toBeGreaterThanOrEqual(0)
        })
    ...
    describe(getMigrationOptions, () => {
        it("returns a copy (not the internal reference)", () => {
            const options1 = getMigrationOptions()
            const options2 = getMigrationOptions()
            options1.schedule = "low_dependency"
            expect(options2.schedule).toBe("western_standard")
        })

        it("reflects options set by setMigrationOptions", () => {
            setMigrationOptions({
                schedule: "low_dependency",
                direction: "receiving",
            })
            const options = getMigrationOptions()
            expect(options.schedule).toBe("low_dependency")
            expect(options.direction).toBe("receiving")
        })
    })
})
