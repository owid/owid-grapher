...
import { formatCount, formatShare } from "../core/CausesOfDeathHelpers.js"
import { useCausesOfDeathChartContext } from "../core/CausesOfDeathContext.js"
import { TreeNode } from "../core/CausesOfDeathConstants.js"
        ...
        <g fill={textColor} style={{ pointerEvents: "none" }}>
            ...
            {textWrap.description &&
                ...
                shouldShow.description && (
                    <MarkdownTextWrapSvg
                        textWrap={textWrap.description}
                        x={bounds.description.x}
                        y={bounds.description.y}
                        fillOpacity={0.7}
                    />
                )}
            {textWrap.perYear && bounds.perYear && shouldShow.perYear && (
                <MarkdownTextWrapSvg
                    textWrap={textWrap.perYear}
                    x={bounds.perYear.x}
                    y={bounds.perYear.y}
                    fillOpacity={0.7}
                />
            )}
            {textWrap.perDay && bounds.perDay && shouldShow.perDay && (
                <MarkdownTextWrapSvg
                    textWrap={textWrap.perDay}
                    x={bounds.perDay.x}
                    y={bounds.perDay.y}
                    fillOpacity={0.7}
                />
