...
import { PlacedGroup } from "../helpers/PovertyGroupAnnotationsHelpers.js"

import { BezierArrow } from "@ourworldindata/grapher"

export function PovertyGroupAnnotations({
    placedAnnotations,
}: {
    placedAnnotations: PlacedGroup[]
}): React.ReactElement | null {
    if (placedAnnotations.length === 0) return null

    return (
        <g className="group-annotations">
            {placedAnnotations.map((placedAnnotation) => (
                <PovertyGroupAnnotation
...
function PovertyGroupAnnotation({
    placedAnnotation: {
        textAnchor: anchor,
        placement: position,
        bounds,
