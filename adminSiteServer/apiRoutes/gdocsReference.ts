...
import { getEntitiesForProfile } from "@ourworldindata/utils"
    ...
    for (const row of rows) {
        ...
        const entry: ComponentUsageByDocType = {
            ...
            label: componentUsageLabel(
                totalDocs > 0 ? docsUsingIt / totalDocs : 0
            ),
        }
...
export async function getComponentInstances(
    ...
    const docTypeFilter = stringParam(req.query.docType)
    const variationFilter = stringParam(req.query.variation)
    ...
    const parsed = rows.map((row) => {
        ...
        return { row, config, minimal: minimizeConfig(config) }
    })
    const valueProps = new Set(component.valueProps ?? [])
    const allParts = parsed.map((instance) =>
        partsOf(instance.minimal, valueProps)
    )
    // Only cleanly parsed instances say anything about what the component
    // needs: a malformed one (legacy shape) keeps its props verbatim, so it
    // still gets a signature and shows as a form, but stays out of every
    // denominator — required-ness, adoption, defaults, universal parts.
    const isClean = (instance: {
        minimal: MinimalBlock | undefined
    }): boolean => instance.minimal?.clean === true
    const cleanParts = allParts.filter((_, index) => isClean(parsed[index]))
    const analyzableCount = cleanParts.length
    const universal = universalParts(cleanParts, analyzableCount)
    ...
    for (const parts of cleanParts) {
        const names = new Set(parts.map((part) => part.split(":")[0]))
        for (const name of names)
            propAdoption[name] = (propAdoption[name] ?? 0) + 1
    }

    // Per-prop defaults: what the parser filled in for a prop the author left
    // out — read off the stored (enriched) config of instances whose minimal
    // source lacks it. Most common value wins when extractions disagree.
    const defaultCounts = new Map<string, Map<string, number>>()
    for (const { config, minimal } of parsed) {
        if (!config || !minimal?.clean) continue
        for (const prop of component.props) {
            if (prop.name in minimal.props) continue
            const v = config[prop.name]
            if (
                typeof v !== "string" &&
                typeof v !== "number" &&
                typeof v !== "boolean"
                ...
                continue
            // An empty string is the parser saying "nothing", not a default
            if (typeof v === "string" && v.trim() === "") continue
            const counts = defaultCounts.get(prop.name) ?? new Map()
            counts.set(String(v), (counts.get(String(v)) ?? 0) + 1)
            defaultCounts.set(prop.name, counts)
        }
    }
    const propDefaults: Record<string, string> = {}
    for (const [name, counts] of defaultCounts) {
        propDefaults[name] = [...counts.entries()].sort(
            (a, b) => b[1] - a[1]
        )[0][0]
    }
    ...
    for (const ref of component.pinned ?? []) {
        const inDoc = scanned
    ...
    const syntheticExamples: SyntheticExampleInfo[] = component.examples.map(
        (example, exampleIndex) => {
            ...
            try {
                ...
                // An unparsable example keeps the empty signature — which is
                // also the legitimate signature of the default form, so it
                // must not be looked up: it would report the broken example
                // as observed and hide it from the "not yet used" list, the
                // one place whoever edits the sidecar would notice it.
            }
            return {
                exampleIndex,
                archie: example.archie,
                signature,
                observed: parsed && bySignature.has(signature),
            }
        }
    )
...
export async function getTemplateExemplars(
    ...
    const template = (templatesRegistry as TemplateReference[]).find(
    ...
    for (const slug of template.exemplars ?? []) {
        const [row] = await db.knexRaw<{
            ...
            SELECT
                ...
                content->>'$.scope' AS scope,
                content->>'$.exclude' AS exclude
            FROM posts_gdocs
            WHERE slug = ? AND published = 1 AND publishedAt <= NOW() AND type = ?
            LIMIT 1`,
            [slug, templateId]
        )
        if (!row) {
        ...
        // Profiles bake per entity: resolve the first entity in scope so the
        // live link and the rendered preview have one to instantiate.
        let entitySlug: string | undefined
        if (templateId === OwidGdocType.Profile && row.scope) {
            const entities = getEntitiesForProfile(
                row.scope,
                row.exclude ?? undefined
            )
            if (entities.length > 0) entitySlug = profileEntitySlug(entities[0])
        }
        ...
        exemplars.push({
            ...
            docType: row.docType as OwidGdocType,
            publishedAt: toIsoOrNull(row.publishedAt),
            ...(entitySlug !== undefined && { entitySlug }),
        })
    }
