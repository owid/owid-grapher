...
export async function handlePostMultiDimRedirect(
    ...
    const publishedChartExists = await trx("chart_configs")
    ...
    const publishedExplorerExists = await trx(ExplorersTableName)
        .select(trx.raw("1"))
        .where({ slug: sourceSlug, isPublished: true })
        .first()

    const publishedMultiDimExists = await trx(MultiDimDataPagesTableName)
        .select(trx.raw("1"))
