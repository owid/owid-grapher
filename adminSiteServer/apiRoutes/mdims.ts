...
async function validateNoRedirectConflict(
    ...
    if (existingMultiDimRedirect) {
        throw new JsonError(
            `A multi-dim redirect with source '${source}' already exists`,
            400
        )
    }
...
export async function handlePatchMultiDim(
    ...
    if (slug !== undefined && slug !== multiDim.slug) {
        ...
        await validateNoRedirectConflict(trx, newSlugSource)
...
export async function handlePostMultiDimRedirect(
    ...
    await validateNoRedirectConflict(trx, source)
    const sourceSlug = Url.fromURL(source).slug
