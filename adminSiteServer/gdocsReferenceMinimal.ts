...
export interface MinimalBlock {
    ...
    /**
     * Whether the block parsed without errors, so defaults were stripped and
     * the props say something about what the component needs. A malformed
     * block (legacy shape, unknown keys) keeps its props verbatim and proves
     * nothing about required-ness or defaults.
     */
    clean: boolean
}
...
export function minimizeRaw(raw: OwidRawGdocBlock): MinimalBlock {
    ...
    const clean =
        baseEnriched !== undefined &&
        (baseEnriched.parseErrors ?? []).length === 0
    if (clean) {
    ...
    return { raw, props: authored, clean }
}
