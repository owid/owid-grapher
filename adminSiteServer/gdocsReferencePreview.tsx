...
import type { GdocBase } from "../db/model/Gdoc/GdocBase.js"
...
.topic-page-header,
.about-header { display: none !important; }
...
// What the page renders: a loaded gdoc class, or a profile instantiated for
// an entity (a plain interface — the attachment fields are then optional).
type PreviewGdoc =
    | SyntheticGdoc
    | (OwidGdocData & Partial<Pick<GdocBase, "donors" | "latestDataInsights">>)
        ...
        <Html>
            ...
            <body>
                ...
                <script
                    type="module"
                    dangerouslySetInnerHTML={{
                        __html: `window.runSiteFooterScripts(${serializeJSONForInlineScript(
                            {
...
function getExampleArchie(
    component: { id: string; examples: { archie: string }[] },
    exampleParam: string | undefined
): string {
    // Number, not parseInt: "1.9" and "1abc" must 404 rather than silently
    // previewing example 1.
    const index = exampleParam === undefined ? 0 : Number(exampleParam)
    const example =
    ...
    if (!example)
        throw new JsonError(
            `"${component.id}" has no example at index "${exampleParam}"`,
            404
        )
...
// Blocks that only occur on about pages (team, donors). Their layout entries
// live under the "about-page" container — in an article they fall back to
// the text column, which squeezes a row of person cards into six columns.
const ABOUT_PAGE_BLOCK_TYPES = ["people", "people-rows", "person", "donors"]

// A component example is wrapped as an article; some blocks only render as
// they do on the site when the document is of the type they belong to. The
// linear-topic-page TOC derives its sections from h1s, which only happens on
// a linear topic page; about-page blocks take their layout from the about
// page container.
function applyPreviewDocumentType(content: OwidGdocPostContent): void {
    const blockTypes = collectBlockTypes(content)
    if (blockTypes.has("ltp-toc")) {
        content.type = OwidGdocType.LinearTopicPage
    } else if (ABOUT_PAGE_BLOCK_TYPES.some((type) => blockTypes.has(type))) {
        // OwidGdocPostContent's type union stops at posts; the about-page
        // class (GdocAbout) is picked by gdocFromJSON from this field.
        Object.assign(content, {
            type: OwidGdocType.AboutPage,
            "hide-nav": true,
        })
        // A person card carries no layout of its own — on the site it only
        // ever sits inside a people list, so preview a bare one that way.
        content.body = content.body?.map((block) =>
            block.type === "person"
                ? ({
                      type: "people",
                      items: [block],
                      parseErrors: [],
                  } satisfies EnrichedBlockPeople)
                : block
        )
    }
}

// Blocks that read the document's topic tag: the `tags[0]?.name` readers in
// site/gdocs/components/ArticleBlock.tsx, which render an error placeholder
// on an untagged document. Keep the two in step. The tag is attached before
// loadState because loadRelatedCharts (for all-charts) reads it.
const TAG_DEPENDENT_BLOCK_TYPES = [
    "all-charts",
    "featured-metrics",
...
export async function renderGdocsReferenceComponentPreview(
    ...
    const component = (componentsRegistry as ComponentRegistry).components.find(
        (component) => component.id === componentId
    )
    if (!component)
        throw new JsonError(`No such component: "${componentId}"`, 404)
    const archie = getExampleArchie(component, exampleParam)
...
/**
 * Preview of a whole published document — a template's exemplar rendered
 * with its real page chrome (header, sections, footer matter), as the site
 * renders it. A profile template bakes per entity, so it previews as its
 * `entity` instantiation (the entity slug of the live URL); without one the
 * bare template would render placeholders.
 */
export async function renderGdocsReferenceDocumentPreview(
    gdocId: string,
    entityParam: string | undefined,
    knex: db.KnexReadonlyTransaction
): Promise<string> {
    // getAndLoadGdocById's two halves, split apart: only the missing row is a
    // 404, while a failure to load a document that does exist — a bad type, a
    // broken attachment — surfaces as itself instead of being relabelled
    // "this document doesn't exist". Same queries either way.
    const base = await getGdocBaseObjectById(knex, gdocId, true)
    if (!base) throw new JsonError(`No gdoc with id "${gdocId}"`, 404)
    const loaded: SyntheticGdoc = await loadGdocFromGdocBase(knex, base)
    if (!loaded.published)
        throw new JsonError(`Gdoc "${gdocId}" is not published`, 404)

    let gdoc: PreviewGdoc = loaded
    if (loaded.content.type === OwidGdocType.Profile) {
        const profile = loaded as GdocProfile
        const entity = getEntitiesForProfile(
            profile.content.scope,
            profile.content.exclude
        ).find((candidate) => profileEntitySlug(candidate) === entityParam)
        if (!entity)
            throw new JsonError(
                `Entity "${entityParam}" is not in the scope of profile "${gdocId}"`,
                404
            )
        const instantiated = await instantiateProfileForEntity(
            profile,
            entity,
            {
                knex,
        ...
        if (!checkShouldProfileRender(instantiated.content))
            throw new JsonError(
                `Profile "${gdocId}" has no renderable content for "${entityParam}"`,
                404
            )
        gdoc = instantiated
    }
    ...
    return renderToHtmlPage(
        <GdocsReferencePreviewPage
            gdoc={gdoc}
            pageTitle={`${gdoc.content.title ?? gdoc.slug} — document preview`}
            styles={SHARED_PREVIEW_STYLES}
        />
    )
}
