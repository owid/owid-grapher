...
graphers
    ...
    it("should handle foreign key relationships correctly", async () => {
        ...
        // Test foreign key constraint to explorers table
        const explorerExists = await testKnexInstance!(ExplorersTableName)
            .where({ slug: testExplorerSlug })
            .first()
        expect(explorerExists).toBeTruthy()

        // Verify all explorer views reference the correct explorer
        const views = await testKnexInstance!(ExplorerViewsTableName).where({
            explorerSlug: testExplorerSlug,
        })
        expect(views.length).toBe(viewsCount)

        for (const view of views) {
            expect(view.explorerSlug).toBe(testExplorerSlug)
        }

        // Test foreign key constraint to chart_configs table
        const viewsWithConfigs = views.filter((v) => v.chartConfigId)
        for (const view of viewsWithConfigs) {
            const configExists = await verifyChartConfigExists(
                view.chartConfigId
            )
