...
// If using wp eval-file, pass "--user=[USER_ID]" flag (e.g. lando wp eval-file --user=35 /path/to/script.php)
//
// Check out the following commits for other instances of that scripts:
...
function remove_has_2_columns()
{
  ...
  foreach ($posts as $post) {
    ...
    $patterns = array(
      ...
      "/<!-- wp:columns {\"className\":\"has-2-columns is-style-sticky-right\"} -->/"
    );
    $replacements = array(
      "wp-block-columns",
      "<!-- wp:columns -->",
      "<!-- wp:columns {\"className\":\"is-style-sticky-right\"} -->"
    );
