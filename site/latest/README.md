...
Data insights vary by _where_ they render, not by authoring. In the unfiltered feed they're a condensed teaser linking to their page. With the data-insight type filter on, the feed offers a **View: Expanded / Compact** toggle ([`LatestViewToggle`](./LatestViewToggle.tsx)) — a grid item of its own, rendered before the cards it governs, sitting at the top of the right-hand column above the newsletter block on desktop and at the top of the page on mobile. Expanded shows each insight whole, read in place ([`LatestDataInsightExpanded`](./LatestDataInsightExpanded.tsx)); Compact shows the very same condensed teaser as the unfiltered feed, linking out — one card design, one behaviour, wherever it appears. (An earlier iteration had a third, compact-but-expandable-in-place card; it was dropped because its faded-out bottom looked like the teaser's yet did something else, and switching to it barely changed anything above the fold.) The toggle is local UI state, not URL state. It renders only for the type filters that offer it, so its absence is what says "not applicable here" — which means the feeds that have one sit 40px lower on mobile, where the toggle takes a row of its own. The baked skeleton can't know the active filter (one static shell serves every query string), so it renders no toggle and a deep-linked filtered feed gains one when the app mounts. Which type filters offer it is a list, `LATEST_TYPES_WITH_VIEW_TOGGLE` in [`latestUtils.ts`](./latestUtils.ts), so extending it to data updates is a matter of adding the type and having its hit component honour `view`. The expanded card needs the authors' avatars, which is why the indexer loads `linkedAuthors` for data insights.
...
`exp-latest-sticky-filters-v1` (registered in `packages/@ourworldindata/utils/src/experiments/config.ts`) trials keeping the filters in reach while scrolling: control, "reveal on scroll up", and "fully sticky". Each arm's layout is pure CSS keyed off the `exp-latest-sticky-filters-v1--<arm>` body class the edge middleware adds (see [`LatestSearch.scss`](./LatestSearch.scss)), so the baked skeleton and the mounted app look identical and nothing flashes when the app takes over. The sticky element is the facets grid item, not the facets themselves: a sticky element only moves within its parent's box. On mobile the content-type dropdown moves above the topic pills, the pills gain a "Filter by topic" label (rendered in every arm, shown by CSS), and a negative `top` lets the dropdown scroll out so only the pills stay pinned.

Alongside the experiment (in every arm, so it isn't a confound), the topic pills render most-popular-first per `LATEST_TOPIC_AREAS_BY_POPULARITY` in [`latestUtils.ts`](./latestUtils.ts), a hardcoded ranking from how often each topic filter was actually set on `/latest` (GA `owid.site_latest` events, see the constant's comment for the source and the path to making it live); areas not in the list fall to the end in tag-graph order.

The reveal-on-scroll-up arm is the one piece CSS can't express, and only just: `useRevealOnScrollUp` in [`latestHooks.ts`](./latestHooks.ts) tracks the scroll direction and nothing else. The bar is sticky at one of two offsets — parked a bar-height above the pin point, out of view, or at the pin point — and transitioning `top` between them is the slide. It therefore renders at `max(its place in the flow, top)` in every state and can never be displaced from where the flow puts it, which is what a transform-based reveal gets wrong: the transform keeps applying once the bar stops being stuck, dragging it down over the content.

Two custom properties carry the offsets: `--latest-sticky-filters-top` (0 on desktop, negative on mobile so only the pills stay) and `--latest-sticky-filters-height`, published by the hook since CSS can't measure it. The height has no fallback on purpose — until it lands, `top` is invalid and computes to `auto`, so the bar never sticks. The padding around the bar applies in both states, since the parked offset depends on the height. The arm is read once on mount via `getExperimentState()`, and `SiteAnalytics` tags every `/latest` event with it as `experimentArm`.
