...
export default function MultiDimEmbedSettingsPanel({
    ...
    const toggleButtonWidthRef = useRef<number>(90) // Fallback width.
    ...
    // Measure and store toggle button width on initial render. It changes size
    // when the label changes after the dropdowns are expanded. We must use only
    // the size it has when collapsed.
    useEffect(() => {
        const dropdownsContainer = dropdownsRef.current
        if (!dropdownsContainer) return

        const toggleButton = dropdownsContainer.querySelector(
            ".md-dropdowns__toggle"
        ) as HTMLElement

        if (toggleButton) {
            toggleButtonWidthRef.current = toggleButton.offsetWidth
        }
    }, [])
    ...
    const calculateCollapsedCount = useCallback(() => {
        ...
        const toggleButtonWidth = toggleButtonWidthRef.current
        ...
        dropdowns.forEach((dropdown, index) => {
            originalDisplayValues[index] = dropdown.style.display
        ...
        dropdowns.forEach((dropdown, index) => {
            if (originalDisplayValues[index] !== undefined) {
                dropdown.style.display = originalDisplayValues[index]
            }
        })
