...
function SearchChartHitMediumRichData({
    ...
    const { data: fetchedChartConfig, status: loadingStatusConfig } = useQuery({
        queryKey: chartHitQueryKeys.chartConfig(hit.slug),
        queryFn: () => {
    ...
    const { data: inputTable, status: loadingStatusData } = useQuery({
        queryKey: chartHitQueryKeys.chartData(hit.slug),
        queryFn: () =>
            fetchInputTableForConfig({
...
function CaptionedThumbnail({
    ...
    if (
        tab === GRAPHER_TAB_NAMES.WorldMap &&
        grapherState.map.time &&
        grapherState.map.time !== grapherState.endTime
    ) {
        const mapTime = grapherState.table.timeColumn.formatTime(
            grapherState.map.time
        )
        caption += ` (${mapTime})`
    }
...
function pickEntitiesForDisplay({
    ...
    return match(grapherState.addCountryMode)
        ...
        .with(EntitySelectionMode.MultipleEntities, () => {
            // If entities are plotted, fill up the remaining slots with the default ones.
            // Don't do that if columns are plotted since Grapher would switch to faceting mode
            ...
            return isEntityStrategy
                ? R.unique([...pickedEntities, ...defaultEntities])
                : pickedEntities
        })
        .exhaustive()
}
