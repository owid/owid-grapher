...
import { SynonymMap } from "@ourworldindata/search-types"
...
describe(buildSynonymMap, () => {
    ...
    it("should handle terms that appear in multiple synonym groups", () => {
        // "population" appears in both pop group and people group
        const populationSynonyms = synonymMap.get("population")

        expect(populationSynonyms).toContain("pop")
        expect(populationSynonyms).toContain("people")
    })
