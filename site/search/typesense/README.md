...
Both collections reference the `owid-synonyms` [synonym set](https://typesense.org/docs/30.2/api/synonyms.html) via `synonym_sets` in their schemas, so every query applies it automatically — no per-query parameter needed. The set is upserted by the indexers (`ensureSynonymSet` in `baker/typesense/typesenseSynonyms.ts`) and mirrors the Algolia synonym configuration exactly:
