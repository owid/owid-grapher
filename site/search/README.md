...
When a reader has a whole topic in view and nothing narrowing it, `SearchTopicKeywordLinks` offers that topic's curated keywords as a "Refine search:" line of links under the search bar, and clicking one sets the query to that keyword while keeping the topic as a filter (`?q=solar&topics=Energy`), so the narrowing is visible and removable. Links rather than pills, because a pill next to the search bar reads as something you could swap the topic for, while clicking one in fact adds a term inside the topic. The line is hidden on the writing tab: the keywords are generated from the charts in a topic, so they narrow data and say nothing about the articles.
