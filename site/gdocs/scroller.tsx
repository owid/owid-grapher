...
export default function Scroller ({ d, styles }: any) {
  ...
  return <section className={'stickySection'}>
    {
      figureSrc ? 
        <div className={'stickyFigure'}>
          <iframe src={figureSrc} loading="lazy" style={{ width: '100%', height: '550px', border: '0px none' }} />
        </div> : null
    }
    <div className={'stickyContent'}>
      {d.value.filter((_d: any) => _d.type === 'text').map(({value}: any, i: any) => {
