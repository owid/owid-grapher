...
export default function ArticleElement ({ d, styles }: any) {
  ...
  const handleArchie = (d: any, key: any) => {
    ...
    if (_type === "chart") {
      ...
      content = <figure key={key} className={`aside-text ${d.value.position ? d.value.position : ''}`}>
      ...
      const columns = +d.value.find((_d: any) => _d.type === "columns").value;
  
      return <div key={key} className={'chartGrid'} style={{display: 'grid', gridTemplateRows: 'auto', gridTemplateColumns: `repeat(${columns}, 1fr)`}}>
        {d.value.filter((_d: any) => _d.type === 'chart').map((_d: any, i: any) => {
  ...
  if (d.type === "text") {
    ...
    return <div dangerouslySetInnerHTML={{ __html: 
      (d.value.startsWith('<div') || d.value.trim() === '<hr />') ? d.value.replace(/\\:/g, ':') : `<p>${d.value.replace(/\\:/g, ':')}</p>` }} />
  } else {
