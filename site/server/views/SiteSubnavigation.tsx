...
const subnavs: { [subnavId: string]: SubnavItem[] } = {
    "about": [
        ...
        { label: "How-Tos", href: "/about/how-to-use-our-world-in-data", id: "how-tos" },
...
export class SiteSubnavigation extends React.Component<{ subnavId: string, subnavCurrentId?: string }> {
    render() {
        ...
        if (subnavLinks) {
            return <div className="site-subnavigation">
                <div className="site-subnavigation-scroll">
                    <ul className="site-subnavigation-links">
                        {subnavLinks.map(({ href, label, id }) => <li className={id === subnavCurrentId ? "current" : undefined} key={href}><a href={href}>{label}</a></li>)}
                    </ul>
                </div>
            </div>
        } else {
