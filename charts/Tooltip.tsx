...
export class TooltipView extends React.Component {
    ...
    @computed get rendered() {
        ...
        const tooltipStyle: React.CSSProperties = { position: 'absolute', pointerEvents: 'none', left: `${x}px`, top: `${y}px`, whiteSpace: 'nowrap', backgroundColor: "rgba(255,255,255,0.92)", boxShadow: '0 2px 2px rgba(0,0,0,.12), 0 0 1px rgba(0,0,0,.35)', borderRadius: "2px", textAlign: 'left', fontSize: '0.9em' }

        return <div ref={this.base} className="Tooltip" style={extend(tooltipStyle, tooltip.style || {})}>
            {tooltip.children}
        </div>
    }
