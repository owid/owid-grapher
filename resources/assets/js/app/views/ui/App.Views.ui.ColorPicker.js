;( function() {
	...
	App.Views.UI.ColorPicker = function() {
		...
		this.init = function( $el, data ) {
			...
			this.$div = $( "<div class='" + App.Views.UI.ColorPicker.WRAPPER_CLASS + "'><a href='#' class='close-btn pull-right'><i class='fa fa-times'></i></a><ul class='no-bullets'>" + lisString + "</ul></div>" );
			$el.append( this.$div );
			$lis = this.$div.find( "li" );

			$closeBtn = this.$div.find( ".close-btn" );
			...
			$closeBtn.on( "mousedown", this.close );
