...
export const SECRET_KEY: string = ENV === "production" ? expect('SECRET_KEY') : "not a very secret key at all"
export const SESSION_COOKIE_AGE: number = process.env.SESSION_COOKIE_AGE ? parseInt(process.env.SESSION_COOKIE_AGE) : 1209600
...
export const EMAIL_PORT: number = process.env.EMAIL_PORT ? parseInt(process.env.EMAIL_PORT) : 443
...
export const SLACK_ERRORS_WEBHOOK_URL: string = process.env.SLACK_ERRORS_WEBHOOK_URL || ""

// Where the git exports go
export const GIT_DATASETS_DIR: string = process.env.GIT_DATASETS_DIR || path.join(BASE_DIR, "datasetsExport")
