...
// "Sticky filters experiment" section of site/latest/README.md.
Given(
 ...
 *
 * Scrolled down, the bar is gone entirely in the not-sticky arm, fully
 * pinned in the fully-sticky one, and parked half off the top in the reveal
 * arm, which is why "partly" and "fully" have to be separate steps.
 */
Then("the filters are fully in view", async ({ page }) => {
    await expect(getFacetsContainer(page)).toBeInViewport({ ratio: 1 })
})

Then("the filters are only partly in view", async ({ page }) => {
    await expect(getFacetsContainer(page)).toBeInViewport()
    await expect(getFacetsContainer(page)).not.toBeInViewport({ ratio: 1 })
})
